...
|---|---|---|---|---|
| **E-1** | LoC reduction | Phase 8 close (was "Phase 6 close" — corrected 2026-05-18: Phase 6 was the addition peak; Phase 8 is when legacy provider/streaming/dispatcher deletion ships) | `cloc packages/engine/src` before vs after | Final target -38% (21,800 → 13,250 non-test LoC). Mid-progress check at Phase 6 close (2026-05-17, post-Phase-6G): **24,595 non-test LoC** (89 source files → 124 after Phase 4 + 5 + 5.5 + 6 + 6G + 7 prototype adds; +13% vs baseline). Pre-deletion expansion expected — net delete lands when Phase 8 retires `providers/anthropic.ts` (612), legacy `streaming.ts` (158), `EarlyToolDispatcher` (206), legacy `QueryEngine` in favor of v2 `AISDKEngine`. |
...
| **S-7** | Skills as marketing surface | ✅ **REALIZED** Phase 6 close (2026-05-17, engine v2.3.0 + @t2000/mcp same-version bump) | `t2000-skills/skills/` public repo + `@t2000/mcp` distribution to Cursor / Claude Desktop / claude-code. 14 skills (`t2000-account-report`, `t2000-borrow`, `t2000-check-balance`, `t2000-contacts`, `t2000-engine`, `t2000-mcp`, `t2000-pay`, `t2000-rebalance`, `t2000-receive`, `t2000-repay`, `t2000-safeguards`, `t2000-save`, `t2000-send`, `t2000-withdraw`) baked into `@t2000/mcp` via `tsup` `__BAKED_SKILLS__` define + auto-registered as `skill-*` MCP prompts by `registerSkillPrompts()` in `packages/mcp/src/skills-prompts.ts` (no runtime filesystem reads, no sibling-directory path resolution). | Discovery channel measurable via repo traffic. Marketing surface live: install via `npx @t2000/mcp` exposes 14 skill prompts in any MCP-aware client. |
...
| **F-4** | `prepareStep` (per-step tool gating) | Phase 7 | LLM injection: system → financial_context → memory → skill → user message | All 5 layers in correct order — ✅ **REALIZED 2026-05-18 (engine v2.7.0)**. Pinned by `packages/engine/src/memory/five-layer-ordering.test.ts` (9 tests: F-4 ordering + per-turn caching + degradation + direct prepareStep unit tests for cache guard). Audric dry-run integration ships same day as S.153 (audric `363e4f1`), opt-in via `ENGINE_MEMORY_PATH_ENABLED` flag. |
...
| **F-8** | v0.7b option creation (engine deletion path open) | ✅ **REALIZED** 2026-05-18 (V07B_ROADMAP_DRAFT.md drafted) | `V07B_ROADMAP_DRAFT.md` shipped with the v0.7a tail summary, open-question matrix, D-1 → D-7 decision points, and an explicit "What v0.7b actually IS now" section. Go/no-go decision criteria captured per-row (e.g. Path A trigger: ~2 weeks of `[stream-resume]` logs showing `mid_tool` >5%; Slice B trigger: Audric commits to UIMessage; Phase 7 production: MemWal smoke 2026-05-29 + fallback eval 2026-06-12 + Plan B pivot day 2026-06-26). | Option exists; exercise discretionary. D-4 + D-6 + D-7 already shipped from the v0.7b backlog without forcing full v0.7b commitment. |
...
| **F-10** | Cross-tool composability (skills consumable by Cursor/Claude/audric/CLI) | ✅ **REALIZED** Phase 6 close (2026-05-17, engine v2.3.0) + Phase 6G (engine v2.4.0) | Same skill files in `t2000-skills/skills/` consumed by 4+ MCP-aware clients via `@t2000/mcp`'s `__BAKED_SKILLS__` distribution: (1) **Cursor** — `npx @t2000/mcp` registers `skill-*` prompts as slash-commands; (2) **Claude Desktop** — same MCP server config; (3) **claude-code CLI** — same; (4) **audric/web** — the 14 skill bodies feed the engine's system-prompt assembly via `McpPromptAdapter` (engine v2.1.0). Phase 6G adds the drift firewall: `packages/mcp/src/prompts.ts` 13 hand-rolled workflow prompts now compose against the same skill bodies via `composeSkillBody()` / `composeSkillSections()` — editing a skill propagates automatically to every dependent workflow prompt. Legacy `RecipeRegistry` deleted in Phase 6 (engine v2.3.0) — single source of truth for agent capabilities. | 1 source of truth (`t2000-skills/skills/*/SKILL.md`), N consumers — verified by `packages/mcp/src/test-load-skills.ts` runtime probe + `compose-skills.test.ts` + `skills-prompts.test.ts` + `prompts-compose.test.ts`. |
...
- [x] **File MystenLabs/MemWal GitHub issue with full smoke output — DONE 2026-05-15 ~16:25 AEST as [MystenLabs/MemWal#159](https://github.com/MystenLabs/MemWal/issues/159).** Same-day re-run comment posted ~1.5h later (07:59 UTC) showed Mysten had already patched the original `Enoki dry_run_failed: balance::split MoveAbort` server-side: ingest success **0/10 → 7/10**, retrieves **10/10 → 4/10 (cascade-aborted by a separate sidecar drop-out)**. Original headline error is no longer reproducing; remaining failures are a distinct sidecar-encryption failure mode (`http://localhost:9000/seal/encrypt` drop-out + 60s timeout on 3/10 ingests) that will get its own issue if it persists through the 2026-05-29 checkpoint. Ingest latency p50 25s / p95 42s — usable for off-turn-boundary background writes (Phase 7 design absorbs this), surprising relative to the original 200ms p95 target which was retrieval-only. Issue still OPEN, awaiting Mysten triage/close.
...
- **Phase 6 close (2026-05-17, engine v2.3.0 + 2.4.0):** verify **F-10**, **S-7** — ✅ **VERIFIED 2026-05-18**: 14 skills baked into `@t2000/mcp` via tsup `__BAKED_SKILLS__` define; `registerSkillPrompts()` auto-exposes them as `skill-*` MCP prompts on server start; `McpPromptAdapter` (engine v2.1.0) lets the engine consume them as `prepareStep.system` text. Phase 6G (engine v2.4.0) adds the drift firewall — `mcp/prompts.ts` workflows compose against skill bodies via `composeSkillBody()`. Legacy `RecipeRegistry` deleted (engine v2.3.0), `classifyEffort` decoupled from `matchedRecipe`. Audric consumes the same skills via `@t2000/mcp` dep. **One source of truth (`t2000-skills/skills/*/SKILL.md`), 4+ consumers (Cursor / Claude Desktop / claude-code CLI / audric/web).** Per-phase verification gates: `compose-skills.test.ts` + `skills-prompts.test.ts` + `prompts-compose.test.ts` + `prompt-adapter.test.ts` all green.
- **Phase 7 engine prototype close (2026-05-18, engine v2.7.0):** verify **F-4** — ✅ **VERIFIED 2026-05-18**: `prepareStep` wired in `runStream` with F-4 5-layer ordering (`base → financial_context → memory → skill → user`); `MemoryStore` interface + `InMemoryMemoryStore` reference mock + `EngineConfig.{memoryStore,financialContextBlock,skillRecipeBlock}` + per-turn cache + honest degradation all shipped. `five-layer-ordering.test.ts` (9 tests including direct `buildPrepareStepHook` unit tests for the cache guard) pins the invariant. Full engine suite 1313/0 pass/fail. Scoping doc: `SPEC_PHASE_7_DRAFT.md`. Audric dry-run integration shipped same day in S.153 (commit `363e4f1`, audric) — opt-in via `ENGINE_MEMORY_PATH_ENABLED` env flag, default OFF, mock `MemoryStore` returns [] until MemWal stabilizes.
- **Phase 7 production close (MemWal-gated):** verify **O-1**, **F-11**, **F-12**, **S-1**, **S-10**. Gated on (a) MemWal infra stability post-2026-05-29 checkpoint per §"Phase 7 commitment gate decision" above, (b) audric `MemWalMemoryStore` impl swap (replaces `InMemoryMemoryStore` in `engine-factory.ts` — single-line change), (c) daily `remember()` cron writing turn intent + outcome, (d) decommission of the legacy ECS daily Claude inference cron (O-1). Hard deadline 2026-06-26 (Plan B pivot day). When stable, the engine-side dry-run integration (S.153) becomes the production path with zero further engine changes.
