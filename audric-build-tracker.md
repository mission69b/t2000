...
| 2.5.3 | `maxTokens: 2048 → 8192` (configurable) | 0.5h | not started | — | audric | One-line change in `engine-factory.ts`. Current 2048 silently truncates when thinking is enabled — thinking + response can't exceed 2048. Required for RE Phase 1 |
| 2.5.4 | `toolChoice: 'any' → 'auto'` with thinking guard | 0.5h | not started | — | audric | `toolChoice: 'any'` forces tool use every turn, conflicts with thinking. Switch to `'auto'` — engine already handles tool-free turns correctly. Spec: `spec/REASONING_ENGINE.md` constraints table |
