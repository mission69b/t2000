...
| S.7 | Day 7 — Code deletion pass 2 (engine) + ATOMIC 2-repo deploy. Publish `@t2000/engine` (9 tools removed → 40 final). Wait for npm propagation (`npm view`). Pin EXACT version in Audric (NOT `@latest`). Pre-prepared Audric branch | 1d | code-ready, awaiting deploy | S.6 | both | ✅ **Engine code:** deleted `tools/allowance.ts` (4 tools), `tools/schedule.ts` (3 tools), `tools/autonomy.ts` (2 tools — `pattern_status` + `pause_pattern`). Updated `tools/index.ts` arrays + re-exports. Stripped `pausePatternTool` + `patternStatusTool` from root `index.ts`. Removed flag entries from `tool-flags.ts` (`create_schedule`, `cancel_schedule`, `toggle_allowance`, `update_daily_limit`, `update_permissions`). Updated `packages/engine/README.md` tool tables (29 reads + 11 writes = **40 final**). Engine: typecheck ✅, 271 tests ✅, lint 0 errors, build ✅ (ESM 205.93 KB, down from 213). **Audric code:** stripped agent-allowance system-prompt block from `lib/engine/engine-context.ts`, deleted `pattern_status` + `list_schedules` chip rules from `lib/suggested-actions.ts`, cleaned `STEP_ICONS` + `STEP_LABELS` maps in `components/engine/AgentStep.tsx`, replaced dead `CARD_GROUP` dedupe with passthrough in `components/engine/ChatMessage.tsx`, removed `allowance_*` + `pattern_status` renderers + `AllowanceCard`/`ProposalCard` imports from `components/engine/ToolResultCard.tsx`, deleted orphan `cards/AllowanceCard.tsx` + `cards/ProposalCard.tsx`, updated `cards/index.ts`, trimmed stale scheduled-actions flow + `AllowanceCard` example from `audric/CLAUDE.md`. Audric: typecheck ✅, 309 tests ✅, lint 0 errors. MCP + CLI: zero changes needed (consume engine via `getDefaultTools()`). **Pending operator action — see `spec/s7-release-runbook.md`:** push t2000 commit, run `gh workflow run release.yml --field bump=minor` (0.37.2 → 0.38.0), wait for npm propagation, run `pnpm add @t2000/engine@0.38.0 @t2000/sdk@0.38.0` (exact pin) in audric, smoke test live audric.ai. |
...
| S.9 | Day 9 — Contract cleanup. Verify zero allowance objects with non-zero balance on-chain. Delete `allowance.move` from contracts source. DO NOT redeploy Move package | 0.5d | not started | S.8 | t2000 | Module remains on-chain but gone from source of truth. Doc updates moved to Day 13 |
| S.10 | Day 10 — Settings reorg. 5 sections only (Passport, Safety, Memory, Goals, Contacts). Update ToS content. DO NOT clear `tosAcceptedAt` (comms email is sufficient notice) | 1d | not started | S.6 | audric | If lawyer disagrees with no re-accept, add modal in follow-up PR |
| S.11 | Day 11 — Dashboard polish (Option A). 4 elements above fold: balance / greeting / chat input / chip bar. Greeting slides out on first message. HF widget inline only when debt AND HF < 2.0. Mobile pass | 1d | not started | S.10 | audric | Replace stale render tree; no orphaned components |
| S.12 | Day 12 — Internal smoke test + Phase B kickoff. Full E2E: chat / save / send / swap / borrow / repay / activity / AdviceLog / goals / settings. Re-run alignment manifest grep, snapshot diff to `spec/alignment-manifest-day12.md` | 0.5d | not started | S.11 | both | Phase A complete when all success criteria green |

### Phase B — Alignment sweep (Days 13-15)

| # | Task | Effort | Status | Blocked by | Repo | Ref |
|---|------|--------|--------|------------|------|-----|
| S.13 | Day 13 — Internal documentation + READMEs. Both repos: `README.md`, `CLAUDE.md`, `PRODUCT_FACTS.md` (exact tool count from Day 7), `ARCHITECTURE.md`, `audric-roadmap.md`, this tracker, `AUDRIC_2_SPEC.md` → `spec/archive/` with historical header. Per-package READMEs (sdk, cli, engine, mcp, contracts). Audric legal pages (`/terms`, `/privacy`, `/security`) | 1d | not started | S.12 | both | Acceptance: grep manifest returns zero matches in `**/*.md` for deleted feature keywords (excluding rationale + archive) |
| S.14 | Day 14 — External surfaces. t2000.ai (homepage, terminal demo `TabbedTerminal.tsx`, pricing, docs, footer/nav). audric.ai (hero, features grid, "how it works", FAQ, pricing, blog/changelog deprecation notes). Skills: delete 9 obsolete skill dirs from `t2000-skills/skills/` + refresh `t2000-mcp/SKILL.md` tool table. MCP: remove tool registrations from `packages/mcp/src/tools/` + republish | 1d | not started | S.13 | both | Acceptance: click through every public page on both sites, zero references to deleted features |
...
**Critical path:** S.0 ✅ → S.1 ✅ (manifest + audit) → S.2 (outreach prep, ~1h) → S.3 ✅ (disable) → S.4 ✅ (billing) → S.5 ✅ (destructive migration + S.6 folded in: Audric deletion) → S.7 (engine atomic deploy, fragile) → S.8 (server+SDK) → S.9 (contract) + S.10 (settings) || S.11 (dashboard) → S.12 (E2E + Phase B kickoff) → S.13 (internal docs) → S.14 (external surfaces + skills + MCP) → S.15 (briefings + verify + comms send).
