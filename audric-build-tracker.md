...
| S.15 | Day 15 — External briefings + final cross-repo verification + comms send. Mysten Labs briefing (`article-trust-layer.md`) honest rewrite. npm package descriptions refresh. Final 3-grep sweep. Snapshot to `spec/alignment-manifest-final.md`. Send user comms email via direct Resend. Post changelog | 1d | done | S.14 | both | ✅ **Doc + npm work done.** `article-trust-layer.md` rewritten + product taxonomy paragraph. npm package descriptions refreshed (sdk, engine, cli, mcp). Final 3-grep sweep ran clean. `spec/alignment-manifest-final.md` written. **All 4 packages republished as v0.40.0 via `release.yml` minor bump.** GitHub Release + Discord notification fired. Initial comms test (Apr 18) hit Gmail spam — root cause traced to missing DMARC; broadcast paused into S.16. Marked done on S.16 closeout |
| S.16 | Day 16 — Product naming consolidation + email deliverability fix + broadcast. (a) Canonical four-product taxonomy (Audric Finance / Pay / Intelligence / Store) added to `audric-roadmap.md` top + `CLAUDE.md`. (b) Audric homepage S4 renamed Products → **Audric Finance**, Store flagged "Coming soon", LandingNav reordered Finance/Pay/Intelligence/Store. (c) t2000.ai `THREE_PRODUCTS` + `PRIMITIVES` reference the four product names. (d) Engine system prompts (t2000 `prompt.ts`, audric `engine-context.ts` + `engine-factory.ts`) reference Audric Finance/Pay/Intelligence/Store. (e) Root `README.md` one-liner names the four products. (f) `article-trust-layer.md` got "Product structure today" section. (g) Comms email rewrite — subject `"A note about Audric"`, four-product framing, removed bulk-style footer/disclaimer. (h) Audric Pay reframe pass (`b594f39` t2000 + `98f3f7a` audric): Audric Pay = send-USDC primitive, MPP marked internal — not a promoted product. (i) Engine republished twice via `release.yml`: **v0.40.1** (S.16 system prompt change), **v0.40.2** (Audric Pay reframe). All 4 packages live on npm at 0.40.2. (j) Audric pinned exact `0.40.2` in `apps/web/package.json` + lockfile, deployed via Vercel (deploy `audric-hfg3zqvrj`, ● Ready). (k) DMARC live (`v=DMARC1; p=quarantine; rua=mailto:dmarc@audric.ai; pct=100; adkim=s; aspf=s`); Resend tracking disabled on `audric.ai` domain. (l) Retest to `funkiirabu@gmail.com` (Resend `7c3c27ae…`) — landed inbox after Audric Pay copy v2 ("Move money. Free, global, instant. Send USDC, payment links, invoices") replaced the under-claimed v1 ("send USDC. To people, to wallets"). (m) **Broadcast 2026-04-19: 18/18 sent, 0 failures.** Idempotent send-log at `apps/web/scripts/.simplification-comms-sent.json` (gitignored). Subject "A note about Audric", from `Funkii <funkii@audric.ai>` | 1d | done | S.15 | both | ✅ Phase Simplification COMPLETE. Final state: 4-product Audric (Finance / Pay / Intelligence / Store), engine + sdk + cli + mcp at 0.40.2, audric.ai live on the simplified UI, every external surface aligned, comms broadcast delivered |

**Critical path:** S.0 ✅ → S.1 ✅ (manifest + audit) → S.2 (outreach prep, ~1h) → S.3 ✅ (disable) → S.4 ✅ (billing) → S.5 ✅ (destructive migration + S.6 folded in: Audric deletion) → S.7 ✅ (engine atomic deploy) → S.8 ✅ (server+SDK) → S.9 ✅ (contract) + S.10 ✅ (settings) || S.11 ✅ (dashboard) → S.12 ✅ (E2E + Phase B kickoff) → **Phase A COMPLETE** → S.13 ✅ (internal docs) → S.14 ✅ (external surfaces + skills + MCP) → S.15 ✅ (briefings + npm v0.40.0) → S.16 ✅ (naming consolidation + Pay reframe + npm v0.40.2 + DMARC fix + 18-user comms broadcast). **Phase Simplification COMPLETE.**
