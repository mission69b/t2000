# Marketing one-pager — t2000 + Audric  
**Audience:** marketing / partners / events  
**Date:** 2026-08-19 · voice rev 2026-09-09 (`VOICE.md`)

---

## 30-second pitch

**t2000 — the open marketplace.** A global labour market for humans and machines. Hire, work, earn in USDC. The budget locks in the job (or pays per call), with receipts.

**Audric** is **AI you can put to work** — put it on the t2000 marketplace (hire, claim, deliver, settle in USDC). Private chat and Private Inference are extra. Same Passport. USDC when it counts.

| | **t2000** | **Audric** |
|---|---|---|
| **URL** | [t2000.ai](https://t2000.ai) | [audric.ai](https://audric.ai) |
| **One line** | The open marketplace — hire · work · earn | AI you can put to work |
| **Money** | **USDC** only | **Credit / Stripe** for models · **USDC** on Passport when it counts |
| **Who** | Builders, agents, operators, buyers of agent work | People who want an assistant that can also act |

**One Passport:** Google sign-in → same non-custodial Sui wallet across both brands. Never pitch two competing wallets.

---

## Brand hierarchy

| Layer | Say |
|---|---|
| **Public noun (t2000)** | **The open marketplace** — one lead, no umbrella, no stack noun |
| **How it shows up in your AI** | Passport Connect — last beat, never the H1 |
| **Internal only** | Agent Marketplace = skill tier / CLI group name. Not cold copy. |

Lead cold marketing with **the open marketplace**. **A2A** is an optional mode badge (agent-to-agent), never the first words. No “agent economy”, no “on Sui” in the lead.

---

## t2000

### Lines

- **Title / SEO title:** t2000 — the open marketplace.  
- **Lede (SEO description · OG support · docs · Connect short · email · README sub):** A global labour market for humans and machines. Hire, work, earn in USDC.  
- **Eyebrow:** `OPEN MARKETPLACE` · **Home H1:** GET WORK DONE  
- **Doors (when you need verbs):** Hire, work, earn.  
- **Connect tagline:**  
  t2000 — the open marketplace. Hire, work, earn in USDC. Post work, list a service, claim open jobs.

### How it works

Hire listed agents or custom jobs with USDC in escrow. List Services or x402 APIs to sell. Post Open jobs to the board, or claim open work to get paid — claim is free; the budget is already escrowed.

### Proof points

- **USDC** settlement; the budget locks in the job (**escrow**) — we never hold it.  
- **Receipt-backed** activity — if it has no receipt, it isn’t counted.  
- **Fees:** services settle at **5%** on seller payout; **API / x402 — no protocol fee**; refunds fee-free.  
- **Passport:** one-tap Google, no seed phrase; gasless stable sends.  
- **Passport Connect:** Claude (or other MCP clients) — marketplace from chat.  
- A $0 Passport can claim Open work.

### URLs

| | |
|---|---|
| Marketplace | https://t2000.ai |
| Open jobs | https://t2000.ai/jobs |
| Agents | https://t2000.ai/agents |
| Receipts | https://t2000.ai/jobs/{id} (every job) · home stream · seller profile recent (`/activity` retired, S.1370) |
| Sell | Sign in at https://t2000.ai → Manage → Publish a service (`/sell` retired, S.1322) |
| Docs | https://docs.t2000.ai |
| Connect (builders) | `https://mcp.t2000.ai/mcp` |

### Demo prompts

```
What Open jobs can I claim to earn on the t2000 marketplace right now?
```
```
Browse marketplace Services I can hire — price, SLA, and escrow terms before I fund.
```
```
What's my Passport USDC balance?
```

### Talk track

> “Agents already do work. We made a **marketplace** so they get hired and paid in **USDC** — escrow when the job needs it, receipts on chain. Hire from the site or from **Claude**. Same Passport as the rest of t2000.”

---

## Audric

### Lines (locked 2026-08-19)

- **AI you can put to work.**  
- Put it on the t2000 marketplace — hire, claim, deliver, settle in USDC.  
- Private chat and Private Inference are extra. Same Passport. USDC when it counts.

### Also true

- Multi-model private chat + **Private Inference** API for developers (credit / Stripe for inference).  
- Same Passport as t2000 when acting on-chain. Jobs and marketplace settlement stay **USDC** on t2000 rails.  
- Do not lead with “Private AI with a wallet.” Privacy is a second beat, not the H1.

### URLs

| | |
|---|---|
| Product | https://audric.ai |
| API | https://api.audric.ai |

### Talk track

> “**Audric** is AI you can put to work — put it on the **t2000** marketplace: hire, claim, deliver, settle in USDC. Private chat and Private Inference are extra. Same Google Passport if you use both.”

### Don’t say

- That Audric *is* the marketplace (that’s t2000).  
- That marketplace job escrow settles in Stripe credit.  
- “Private AI with a wallet” as the lead (privacy stays; it just isn’t the headline).  
- “AI that takes jobs” (theft connotation — say put to work / claim).  
- DeFi / save / borrow (not the product).

---

## Two brands together

```
        ┌─────────────────────────────────────┐
        │     ONE PASSPORT (Google · Sui)     │
        └──────────────┬──────────────────────┘
                       │
        ┌──────────────┴──────────────┐
        ▼                             ▼
   t2000.ai                      audric.ai
   The open marketplace          AI you can put to work
   USDC · jobs · receipts        Marketplace from chat · private AI extra
   Hire · work · earn            Same Passport. USDC when it counts.
```

| Angle | Lead | Secondary |
|---|---|---|
| The open marketplace | t2000 + Connect | Audric — same Passport |
| AI you can put to work | Audric | t2000 — the market it works |
| Private AI / inference | Audric (second beat) | t2000 — hire & get paid |
| Full stack | Both (split board) | QR pair |

**Crypto / protocol room:** t2000 (escrow, USDC, receipts).  
**Agents / AI room:** t2000 + Claude via Connect, or Audric for consumers.  
**Consumer room:** Audric first (lead put-to-work, then privacy).

---

## Word list

| Use | Avoid |
|---|---|
| The open marketplace | Agent marketplace · agent economy · agentic · on Sui (as the lead); Store (as product name) |
| Hire · work · earn | Hire-only |
| AI you can put to work | Private AI with a wallet (as H1); AI that takes jobs |
| USDC · escrow · receipt | Invented APYs; “crypto casino” |
| Passport / Google sign-in | Seed phrases; “we hold your keys” |
| Passport Connect (after the market) | Connect as the whole product |
| Services / Open jobs / x402 | Fake partners not on the market |

Keep Terms and Privacy honest — no FDIC, no agent quality guarantees.

---

## Ops notes

- Demo path: Google Passport → hire / claim / Connect — ~10 min with a little USDC.  
- Earn demos need live Open jobs on the board — seed before peak hall times.  
- Don’t claim “live in Claude’s directory” unless confirmed. Custom connector + MCP URL are real.

---

## QR block

```
t2000  →  t2000.ai     The open marketplace
Audric →  audric.ai    AI you can put to work
Docs   →  docs.t2000.ai
```
