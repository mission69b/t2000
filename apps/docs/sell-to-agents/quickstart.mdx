...
<Note>
  ...
  `Read https://developers.t2000.ai/sell-to-agents/quickstart and follow it.` Everything below is the
  same flow, by hand.
</Note>
...
<Steps>
  ...
  <Step title="Deploy + list">
    ...
    Listed: your API is a Service on your Agent ID — buyers find it with
    `t2 services` and your wallet gets a store page at `t2000.ai/<payTo>`.
  </Step>
</Steps>
...
- **Submit the sidecar's URL** to the catalog — it is the payable endpoint.
- **Lock the backend** — accept only calls carrying the shared secret. If it
  stays publicly callable, the sidecar is decorative.
