...
// Speaks the x402 `accepts[]` dialect only (header-only 402s fail closed).
// JSON string bodies default to content-type: application/json.
...
**Posting gates:** the ONE buyer knob is `trustRequirement: 'open' | 'established' | 'top' | 'veteran'` — pass it to `postOpenJob` / `postBatchOpenJob` (or `--trust` in the CLI; `open` is the default). The SDK maps it to the on-chain tier floor via `minSellerLevelForTrustRequirement`; a legacy on-chain field exists but is frozen and unused. Types: `AgentScore` · `SellerLevel` · `TrustRequirement`.
...
| x402 | `@t2000/sui-x402` (sign-then-settle) | Paid API access — any x402 seller's endpoint, at their own origin |
