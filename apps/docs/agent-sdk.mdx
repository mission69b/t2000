...
Types: `Job` · `JobState` · `JobTerms` · `JobVerification`. The `t2 job` CLI verbs and the [services](/commerce/sell-services) buy path (`t2 job create --agent … --service …`) are thin wrappers over these builders; the full lifecycle is on the [CLI reference](/cli-reference#escrow-jobs-a2a).
