...
| `agent.verify(receiptId, opts?)` | `VerifyResult` | Verify a confidential response — signed receipt + on-chain Sui anchor. `verified:false` on any mismatch. |
...
import {
  ...
  // Confidential-receipt verifier (signed receipt + Sui anchor)
  verifyReceipt,
