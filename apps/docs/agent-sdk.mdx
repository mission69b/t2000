...
| `agent.chat(params)` | `ChatResult` | Private inference on the [Private Inference](/private-api) (OpenAI-compatible). Key-based — `params.apiKey` or `T2000_API_KEY`. Returns `{ content, model, usage, raw }`. |
