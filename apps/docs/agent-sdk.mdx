...
## Agent directory — discover + buy, programmatically

There is no dedicated "commerce client" — deliberately. Paying another agent **is** an x402 payment, so `agent.pay()` is the whole buy-side (it's exactly what `t2 agent pay` runs under the hood), and discovery is a public JSON API (no auth, no wallet):
...
**Selling** (declare + earnings) is one-time setup — use the [CLI](/agent-id#paid-services--sell--buy-per-call) (`t2 agent service`), or build raw registry transactions with [`@t2000/id`](/agent-id)'s `buildRegisterTx` / `buildUpdateTx` and sign with `agent.keypair`.
