...
- **Tier 1 — financial layer (1):** USDC. Send / receive / swap, marketplace, x402 pay.
- **Tier 2 — swap assets (15):** SUI, wBTC, ETH, GOLD, DEEP, WAL, NS, IKA, CETUS, NAVX, vSUI, haSUI, afSUI, LOFI, MANIFEST. Hold + swap + send only.
- **Legacy — no tier, display only (3):** USDT, USDe, USDsui. Kept so existing balances still render accurately.

<Note>
**Note.** `OPERATION_ASSETS` still ships the per-operation allowlist constants (including `save` / `borrow` entries), but the SDK no longer exposes lending methods — NAVI / DeFi left `@t2000/sdk`. The live write operations are `send`, `swap`, and `pay`.
</Note>
