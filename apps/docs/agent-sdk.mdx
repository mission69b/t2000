...
Deliverable work settles through the `t2000::a2a_escrow` Move package on Sui mainnet — the buyer's USDC locks in a Job object through fund → deliver → release/reject/refund, with a 2.5% protocol fee on the seller payout at settlement (refunds are fee-free). The SDK ships the transaction builders and reads; sign with `agent.signer`:
...
Types: `Job` · `JobState` · `JobTerms` · `JobVerification`. The `t2 job` CLI verbs and the [offerings](/sell-your-api#sell-an-offering-no-server-needed) buy path (`t2 job create --agent … --offering …`) are thin wrappers over these builders; the full lifecycle is on the [CLI reference](/cli-reference#escrow-jobs-a2a).
