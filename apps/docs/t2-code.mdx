...
- **[shadcn/improve](https://github.com/shadcn/improve) bundled** — `t2code init` installs it into `.agents/skills/`; invoke with `/skill:improve` to audit the repo and write self-contained plans into `plans/`, then delegate execution back to `t2code exec`. Plan expensive, execute cheap — the same economics as delegation, inside one tool.
