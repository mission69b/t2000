...
Type a vibe, get a track. Your agent turns a one-line brief into a rich music prompt with Claude, then pays Stable Audio 2.5 to render the actual audio — a real, commercial-use `.wav` you can play and sell. ~$0.25, no API keys, no subscriptions.
...
1. `POST /anthropic/v1/messages` — Claude turns the brief into a vivid music prompt (~$0.012)
2. `POST /fal/fal-ai/stable-audio-25/text-to-audio` — Stable Audio 2.5 renders the track (~$0.24)
...
```
2 calls · ~$0.25 · ~10s · 0 taps
./midnight-rain.wav  (commercial-use, hosted)
```

## Extend it

- Need a cheaper take? **MusicGen via Replicate** (`/replicate/v1/predictions`, $0.024) runs the open MusicGen model — pass its `version` + `{ prompt, duration }` and poll `/replicate/v1/predictions/status` for long renders
