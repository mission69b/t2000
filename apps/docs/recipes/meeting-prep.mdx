...
```
I'm meeting Jensen Huang from NVIDIA about AI infrastructure. Pull a brief on
the company (size, funding/financials), a profile on him (role, background),
and flag anything recent and relevant. Keep it to one page.
```

## What runs

1. `POST /perplexity/v1/chat/completions` — web-grounded company brief with citations (~$0.012)
2. `POST /exa/v1/search` — semantic search for the person's profile + recent public activity (~$0.012)
3. `POST /serper/v1/search` — latest news mentioning both (~$0.012)
4. `POST /anthropic/v1/messages` — Claude synthesizes the one-pager and flags what's relevant (~$0.012)
...
- Run it for every attendee on tomorrow's calendar in parallel — each is an independent ~$0.05 pass
