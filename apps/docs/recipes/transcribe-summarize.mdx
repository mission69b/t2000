...
Drop in a link to a recording — a podcast clip, a voice memo, a meeting export — and your agent transcribes it and hands back the key points. fal.ai's Whisper fetches the URL server-side (so nothing to upload), and Claude distills it. ~$0.024.
...
1. `POST /fal/fal-ai/whisper` — Whisper transcribes the audio URL (~$0.012)
2. `POST /anthropic/v1/messages` — Claude pulls the key points (~$0.012)
