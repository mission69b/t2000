...
description: "Agent writes the message, generates the front, and ships a real postcard via USPS. ~$1.25 end to end."
---
...
You pay ~$1.25 in USDC. A physical postcard shows up in someone's mailbox 3–5 days later.
...
1. `POST /anthropic/v1/messages` — Claude writes the note text (~$0.012)
2. `POST /fal/fal-ai/flux/dev` — Flux renders the card front (~$0.036)
3. `POST /lob/v1/postcards` — Lob prints + mails the physical postcard (~$1.20)
...
const recipient = { name: 'Mum', address: '123 Lochiel Road, Dunedin 9011, NZ' };
...
const postcard = await agent.pay({
  ...
  body: JSON.stringify({
    ...
    to: { name: recipient.name, address_line1: recipient.address },
    from: { name: 'Phillip', address_line1: 'Sender address here' },
    front: imageUrl,
    back: noteText,
    size: '4x6',
  }),
...
```
3 calls · ~$1.25 · 0 taps · queued for USPS delivery in 3–5 days
```
...
- Use **Lob `/v1/letters`** (~$1.80) for multi-page letters instead of postcards
- Verify the destination first with **Lob `/v1/verify`** (~$0.012) to catch typos before paying $1.20
- Swap Flux Dev for **Recraft 20B** (`/fal/fal-ai/recraft-20b`) for vector-style illustrations
- Generate batches: 10 unique cards to 10 friends, ~$12.48 total, one tap-free pass
