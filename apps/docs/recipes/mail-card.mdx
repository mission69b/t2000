...
description: "Agent writes the message, generates the front, and ships a real postcard via USPS. ~$1.04 end to end."
---
...
You pay ~$1.04 in USDC. A physical postcard shows up in someone's mailbox 3–5 days later.
...
1. `POST /anthropic/v1/messages` — Claude writes the note text (~$0.01)
2. `POST /fal/fal-ai/flux/dev` — Flux renders the card front (~$0.03)
3. `POST /lob/v1/postcards` — Lob prints + mails the physical postcard (~$1.00)
...
```
3 calls · ~$1.04 · 0 taps · queued for USPS delivery in 3–5 days
```
...
- Use **Lob `/v1/letters`** (~$1.50) for multi-page letters instead of postcards
- Verify the destination first with **Lob `/v1/verify`** (~$0.01) to catch typos before paying $1
- Swap Flux Dev for **Recraft 20B** (`/fal/fal-ai/recraft-20b`) for vector-style illustrations
- Generate batches: 10 unique cards to 10 friends, ~$10.50 total, one tap-free pass
