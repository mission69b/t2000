...
description: "Agent writes a script with Claude, then executes it via Judge0 to verify the output. Two paid calls, ~$0.02."
---
...
1. `POST /anthropic/v1/messages` — Claude writes Python (~$0.01)
2. `POST /judge0/v1/submissions` — Judge0 executes it in a sandbox (~$0.01)
...
- Use **Judge0 `/v1/languages`** (~$0.01) to discover available runtimes if you want to branch on language
