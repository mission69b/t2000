...
Point your agent at a ticker and it pulls a live quote, the recent price trend, and the headlines driving it, then hands you a tight analyst-style brief. Real market data, machine-synthesized, ~$0.06.

## The prompt

```
Research NVDA for me — current price, the 30-day trend, and what's moving
the stock right now. Give me a short brief.
```

## What runs

1. `POST /alphavantage/v1/search` — resolve the ticker (skip if you already have it) (~$0.012)
2. `POST /alphavantage/v1/quote` — real-time price, change, volume (~$0.012)
3. `POST /alphavantage/v1/daily` — daily OHLCV for the trend (~$0.012)
4. `POST /newsapi/v1/search` — recent headlines on the company (~$0.012)
5. `POST /anthropic/v1/messages` — Claude synthesizes the brief (~$0.012)
