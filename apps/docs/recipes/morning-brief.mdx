...
description: "Chain CoinGecko + NewsAPI + Anthropic over MPP. Three paid calls, ~$0.036, zero API keys."
---
...
3. `POST /anthropic/v1/messages` — Claude 3.5 Sonnet stitches them into prose

Each call returns HTTP 402, your wallet signs a tiny USDC payment, the gateway proxies upstream and returns 200. Total ~$0.036, ~6s end to end.
...
t2 init
t2 receive          # send $1 USDC to this address
...
t2 pay https://mpp.t2000.ai/anthropic/v1/messages \
  ...
  --data "{\"model\":\"claude-3-5-sonnet-20241022\",\"max_tokens\":1024,\"messages\":[{\"role\":\"user\",\"content\":\"Prices: $PRICES. Headlines: $NEWS. Write a 200-word brief.\"}]}"
```
