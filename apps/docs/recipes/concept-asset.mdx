...
Your agent takes a concept, generates a hero image with fal.ai's Flux, writes a 60-second elevator pitch with Claude, and synthesizes the pitch as an MP3 via ElevenLabs — all in one chain, paid as you go in USDC. ~$0.09, ~18 seconds.
...
1. `POST /fal/fal-ai/flux/dev` — Flux image generation (~$0.03)
2. `POST /anthropic/v1/messages` — Claude writes the pitch (~$0.01)
3. `POST /elevenlabs/v1/text-to-speech/:voiceId` — pitch becomes audio (~$0.05)
...
writeFileSync('./pitch.mp3', Buffer.from(audio.body as ArrayBuffer));
```

## Expected output

```
3 calls · ~$0.09 · ~18s · 0 taps
./hero.png · ./pitch.md · ./pitch.mp3
```

## Extend it

- Swap Flux Dev for **Flux Pro** (`/fal/fal-ai/flux-pro`, $0.05) for higher-fidelity output
