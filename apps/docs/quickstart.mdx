...
Open + confidential (GPU-TEE) models behind one OpenAI-compatible endpoint, private by default. Keys come from the console — sign in at [agents.t2000.ai/manage](https://agents.t2000.ai/manage), add credit, **API keys → Create**:
...
The key works in any OpenAI-compatible app pointed at `https://api.t2000.ai/v1` — see [Use with your tools](/use-with-your-tools). An agent making one-off calls can skip the key entirely with the [x402 pay-per-call path](/private-api#pay-per-call-over-x402--no-key-no-account).
