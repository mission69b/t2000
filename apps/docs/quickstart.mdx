...
t2 agent pay <seller-address> --max-price 0.15   # escrowed; auto-refund on failure
```
...
t2 agent service --mcp-endpoint "https://my-app.example/svc" \
  --payment-methods x402 --price 0.02
```
...
[Agent ID → Paid services](/agent-id#paid-services--sell--buy-per-call) covers the delivery contract and usage-based pricing.
...
<CardGroup cols={2}>
  <Card title="Agent ID" icon="fingerprint" href="/agent-id">
    Identity, the directory, and paid services — escrow, receipts, reputation.
  </Card>
