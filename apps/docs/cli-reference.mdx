...
| `t2 job watch <jobId>` | either | Poll state + what you can do now; exits on settle. A seat (the buyer or the claimed seller) also sees the job's delivery thread and, on on-site jobs, `whereExact` (unit, buzzer, which door — never in the hashed spec). |
| `t2 job ping <jobId> [message] [--image <url>…]` | buyer or seller | Post on the job's delivery thread — text (≤500) and/or up to 6 HTTPS photos; parties only, funded / delivered jobs. Logistics, not delivery (`t2 job deliver` still pins the work). |
...
| `t2 job review --ids 0xA,0xB --stars <1-5>` · `--all-unreviewed --stars <1-5>` | buyer | Rate several settled, delivered jobs you bought in ONE transaction — one star value for the wave, all rate or none. `--ids` = the jobs you name (up to 10). `--all-unreviewed` = the **host** picks unreviewed settled deliveries on your buyer seat, first 10, and reports the remainder on a rerun — the CLI never guesses from your inbox. First-time stars only (already-rated ids are skipped), and `--text` is refused on a batch — notes stay on the single-job form. The same wave lands in your AI (`t2000_job_review` with `allUnreviewed`) and on Manage (**Rate all unreviewed (N)**) with the audric follow-up; until that host action ships, the flag answers with a prepare error. |
...
| Command | Who | What it does |
| --- | --- | --- |
| `t2 job open --title <t> --brief <file-or-text> --max <usdc>` | buyer | Post an opening — **escrows the budget now**. `--sla 24h` (min 1h) · `--open-for 24h` · `--trust open\|established\|top\|veteran` · `--category <department>` (the aisle; slug or alias) · `--mode` / `--where` (the place). |
| `t2 job board [query]` | anyone | Read the board, paged (`--limit 24` · `--offset` = the last page's `nextOffset` · `--status`). `--category <department>` (what) and `--mode remote\|on-site\|either` (where) filter it. Rows carry a `briefPreview` — read the full brief before claiming. Public, no wallet. |
