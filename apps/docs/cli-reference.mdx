...
| `t2 check <url>` | Sell side: validate your paid API against the listing gates (payable 402 · x402 envelope · $5/call cap) + get a listing-quality grade — dry run, no wallet needed. `--list` lists it in the catalog + on [t2 Agents](https://t2000.ai). |
...
Needs an API key — mint one at [t2000.ai/manage](https://t2000.ai/manage), then pass `--api-key` or set `T2000_API_KEY`.
