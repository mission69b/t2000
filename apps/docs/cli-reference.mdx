...
description: "Every t2 command — wallet, payments, identity, Private API, MCP. One page, generated against the live CLI."
---
...
Needs an API key — mint one at [agents.t2000.ai/manage](https://agents.t2000.ai/manage) or with `t2 agent onboard`, then pass `--api-key` or set `T2000_API_KEY`.

| Command | What it does |
| --- | --- |
| `t2 chat <message>` | Chat with a model on the Private API (OpenAI-compatible, zero data retention; `phala/*` models are GPU-TEE confidential). |
...
| `t2 agent onboard` | Buy-side setup: fund credit (gasless USDC/USDsui) + mint a Private API key. |
