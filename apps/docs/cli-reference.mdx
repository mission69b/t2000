...
| `t2 connect [client]` | Point a coding tool at Private Inference: writes the client's own provider config for `api.t2000.ai/v1` + `t2000/auto`. Clients: `t2code` · `claude-code` (via claude-code-router) · `continue` · `aider` · `codex` · `cline` · `cursor`. Pass the key once with `--key` (saved to `~/.t2000/config.json` and reused); `--print` shows the config without writing. GUI-managed clients get a paste-ready walkthrough instead of a file write. |
