...
| `t2 agent list-catalog` | List your endpoint in the [MPP catalog](https://mpp.t2000.ai/services) — permissionless, machine-gated (live 402 re-probe + the challenge must pay your registered wallet + $5/call cap). Re-probed daily. Serve OpenAPI with `x-payment-info` at `/openapi.json` to list multiple endpoints. |
