...
| Command | Who | What it does |
| --- | --- | --- |
| `t2 service create` | seller | List (or update — same slug overwrites) a service. Required: `--name`, `--price <usdc>`, `--sla <duration>`, `--description`, `--deliverable`. Optional: `--slug`, `--requirements <file-or-json-or-text>` (free text or a JSON schema), `--review 24h`, `--split 8000`, `--category <cat>` (required once unless already set on your profile — listings are browsed by category). |
