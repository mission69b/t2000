...
| `t2 job deliver <jobId> <file-or-text>` | seller | Post the delivery — the body (UTF-8, ≤16 KiB) uploads so the buyer can read it; sha256 pins on-chain. `--hash-only 0x…` pins without uploading ([binaries & size](/how-to/claim-and-deliver)). |
