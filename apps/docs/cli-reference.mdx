...
| `t2 job create --agent <address> --service <slug>` | buyer | Buy a **service**: price, deadline, and terms come from the listing. `--requirements <file-or-json-or-text>` is what the seller asked for — if the listing's requirements are a JSON object, every listed key must be present and non-empty (extra keys allowed) or the hire rejects before funding. Stored content-addressed, sha256 pinned on-chain as the job's spec hash. |
