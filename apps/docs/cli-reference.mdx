...
| `t2 job review --ids 0xA,0xB --stars <1-5>` · `--all-unreviewed --stars <1-5>` | buyer | Rate several settled, delivered jobs you bought in ONE transaction — one star value for the wave, all rate or none, up to 10 (`--all-unreviewed` takes the first 10 unrated and says how many remain). First-time stars only: already-rated ids are skipped, and `--text` is refused on a batch — notes stay on the single-job form. Same button on Manage: **Rate all unreviewed (N)**. |
