...
API key:   sk-…        (create one at platform.t2000.ai → API keys)
```

<Note>
  Get a key + add credit at **[platform.t2000.ai](https://platform.t2000.ai)** (sign in with Google — same Passport + balance as Audric). Calls are metered per token and fail closed at a $0 balance. Browse models + pricing any time with `GET /v1/models` (public, no key).
</Note>
...
Then pick a model from the catalog (e.g. `zai/glm-5.2`, `anthropic/claude-sonnet-4.6`, or a confidential `phala/glm-5.2`).
...
3. Under **Models**, add the model IDs you want (e.g. `zai/glm-5.2`, `anthropic/claude-sonnet-4.6`), then **Verify**.

<Note>
  Cursor's agentic features are tuned for specific frontier models. For a drop-in private chat model, add a model like `zai/glm-5.2` or `anthropic/claude-sonnet-4.6` and select it in the chat model picker.
</Note>
...
The same base URL + key works in any tool that targets OpenAI Chat Completions — **Continue**, **Cline**, **Aider** (`--openai-api-base`), **LiteLLM** (`openai/<model>`), **LlamaIndex**, and most "bring your own OpenAI key" apps. If a tool has an "OpenAI base URL" or "custom endpoint" field, point it at `https://api.t2000.ai/v1`.
