...
<Note>
  Get a free key at **[agents.t2000.ai/manage](https://agents.t2000.ai/manage)** (sign in with Google — same Passport + balance as Audric). Every account gets a free daily allowance on `moonshotai/kimi-k2.7-code`; paid models are metered per token and fail closed at a $0 balance. Browse models + pricing any time with `GET /v1/models` (public, no key).
</Note>
...
<Note>
  **`t2 connect` needs a Private Inference key** (free at
  [agents.t2000.ai/manage](https://agents.t2000.ai/manage)). That is the path
