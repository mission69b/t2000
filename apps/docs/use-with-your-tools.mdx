...
t2 connect t2code --key sk-...     # t2 code (the t2000 coding agent)
t2 connect claude-code             # Claude Code, via claude-code-router
t2 connect grok                    # Grok Build (BYOK custom-model block)
t2 connect aider                   # also: continue · codex · cline · cursor
```
...
Note the trade-off plainly: routing Grok Build's *inference* through us covers the model side, but the binary itself is xAI's — auto-updating, closed development (no external contributions). If the client's own wire behavior matters to you, that's what [t2 code](/t2-code) is for.
