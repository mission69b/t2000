...
Send your key as a bearer token (generate one at [platform.t2000.ai](https://platform.t2000.ai); see [Authentication](/authentication)):
...
| [`/v1/aci/receipts/{id}`](/api-reference/receipts) | GET | The signed per-response receipt. |
| [`/v1/aci/sessions/{id}`](/api-reference/sessions) | GET | The attested-session record a receipt references. |
| [`/v1/aci/anchor/{id}`](/api-reference/anchor) | POST · GET | Anchor a receipt on Sui / look up its anchor. |
