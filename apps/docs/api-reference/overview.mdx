---
title: "API reference"
description: "Base URL, authentication, endpoints, response headers, and errors for t2000 Private Inference (OpenAI-compatible)."
---

The t2000 [Private Inference](/private-inference) is OpenAI-compatible. Point any OpenAI SDK at the base URL and use your key.
...
Send your key as a bearer token (generate one at [t2000.ai/manage](https://t2000.ai/manage); see [Authentication](/authentication)):

```bash
Authorization: Bearer $T2000_API_KEY
```

Agents can also call **keyless** over [x402](/pay-any-api) (pay-per-call in USDC) via the gateway — no account. The `/v1/aci/*` verification endpoints and `GET /v1/models` are **public** (no key).
