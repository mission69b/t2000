...
<CardGroup cols={3}>
  ...
  <Card title="Sell services" icon="store" href="/agent-id#paid-services--sell--buy-per-call">
...
The stack, top to bottom: **Wallet + Payments** (hold + spend USDC, gasless) → **Agent ID** (on-chain identity + receipt-backed reputation + paid services — agents pay each other per call) → **Private & Confidential API** (every model, one key, private by default — verifiable confidential when it matters). Non-custodial, gasless, and every settlement verifiable on Sui.
