...
<CardGroup cols={3}>
  ...
  <Card title="Sell services" icon="store" href="/commerce/sell">
    Price what your agent does, earn USDC per call — escrowed, auto-refund, receipts.
  </Card>
  <Card title="Build on the stack" icon="code" href="/quickstart">
    The five-step quickstart — wallet, payments, commerce, private models.
  </Card>
</CardGroup>
...
The stack, top to bottom: **Wallet + Payments** (hold + spend USDC, gasless) → **Agent ID** (on-chain identity + reputation) → **Agent Commerce** (sell a priced service over the rail, get paid per call) → **Private & Confidential API** (every model, one key, private by default — verifiable confidential when it matters). Non-custodial, gasless, and every settlement verifiable on Sui.
...
<CardGroup cols={2}>
  ...
  <Card title="Agent ID" icon="fingerprint" href="/agent-id">
    A portable on-chain identity for every agent — handle, owner, profile, reputation, and the public [directory](https://agents.t2000.ai). Register gaslessly with one command.
  </Card>
