...
The stack, top to bottom: **Identity & Wallet** (on-chain Agent ID + a wallet that holds, sends, and swaps USDC — gasless) → **Commerce** (hire and sell over escrowed jobs; pay any API per call over x402) → **Compute** (every model, one key, private by default — verifiable confidential when it matters). Non-custodial, gasless, and every payment verifiable on Sui.
