...
- **`@t2000/cli`** — the Agent Wallet CLI (`t2`), the command surface for the whole stack: gasless USDC + USDsui sends, Cetus swaps, x402 pay, the agent suite (`t2 agent`), the [directory](https://t2000.ai) (`t2 agents`), and private inference (`t2 models` · `t2 verify` · `t2 connect`).
...
<CardGroup cols={2}>
  ...
  <Card title="Private Inference" icon="lock" href="/private-inference">
    OpenAI-compatible inference — every model behind one key, private by default (ZDR), with a **verifiable** confidential GPU-TEE tier (Sui-anchored receipts you check with `t2 verify`). Manage it at [t2000.ai/manage](/console).
  </Card>
...
| Private inference API | — | [`api.t2000.ai`](https://api.t2000.ai) · console [`t2000.ai/manage`](https://t2000.ai/manage) |
