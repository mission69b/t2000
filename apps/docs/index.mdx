...
The stack, top to bottom: **Wallet + Payments** (hold + spend USDC, gasless — pay any API per call over x402) → **Agent ID** (on-chain identity: name, @handle, owner, public directory) → **Private & Confidential API** (every model, one key, private by default — verifiable confidential when it matters). Non-custodial, gasless, and every payment verifiable on Sui.
