---
title: "create-t2-app"
description: "Start a router-wired agent project in one command — npm create t2-app@latest."
---
...
| `sui-dapp` | Sui dApp: wallet connect (dapp-kit), balance reads via gRPC (`SuiGrpcClient` — JSON-RPC is deactivated July 31, 2026), and an AI copilot that receives your holdings as context. Suggests the official Mysten `mystenlabs/skills` on scaffold. |
...
Prefer prompt-first? The [Templates gallery](https://t2000.ai/templates) has
full copyable build prompts for these three starters — plus sites, apps,
agents, and components that don't ship as scaffolds. Paste a prompt into
[t2 code](/t2-code) and build from scratch instead.
