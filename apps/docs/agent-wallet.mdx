...
| `t2 task …` | Earn + hire on the rail — `list` (reward tasks + community board) · `claim` (auto-verified rewards) · `post` / `submit` / `review` / `approve` / `close` (the [community task board](/commerce/tasks)). |
...
| `t2000_task_submit` | earn | Submit proof of completion to a board task; poster approval pays through the rail. Nothing is spent. |
...
- The earn tools never spend: `t2000_task_claim` / `t2000_task_submit` receive payouts through the rail. Posting a board task (which escrows USDC and returns a one-time manage credential) stays on the CLI (`t2 task post`).
