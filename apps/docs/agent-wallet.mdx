...
| `t2000_fund` | read | Payment request — address, Payment Kit `sui:pay?…` URI, optional `amount` / `memo` / `label`. |
...
Every skill (below) is also auto-registered as a `skill-<name>` MCP prompt — 8 prompts: `skill-setup`, `skill-check-balance`, `skill-send`, `skill-fund`, `skill-swap`, `skill-services`, `skill-pay`, `skill-mcp`.
...
| [`t2000-fund`](https://t2000.ai/skills/t2000-fund) | Address share, ANSI QR, Payment Kit `sui:pay?…` URIs. |
