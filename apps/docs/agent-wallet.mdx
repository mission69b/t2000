...
stdio transport — keys never leave the machine. The `env` block is only needed for the inference tools; every wallet tool works without it ([Private Inference keys come from Audric](/private-inference)).
