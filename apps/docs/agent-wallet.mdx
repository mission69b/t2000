...
Every skill (below) is also auto-registered as a `skill-<name>` MCP prompt — 11 prompts: `skill-setup`, `skill-check-balance`, `skill-send`, `skill-receive`, `skill-swap`, `skill-services`, `skill-pay`, `skill-verify`, `skill-mcp`, `skill-hire`, `skill-earn`.
...
| MCP tools | 18 (11 read + 4 write + 2 earn actions + 1 settings) |
| Skills | 11 markdown playbooks shipped as `skill-<name>` MCP prompts |
