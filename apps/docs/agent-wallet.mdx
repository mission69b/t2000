...
- **Models (Audric)** — point a coding tool at [Private Inference](/private-inference), an Audric product (`t2 models` · `t2 connect`)
...
stdio transport — keys never leave the machine. The `env` block is only needed for the model tools; every wallet and marketplace tool works without it ([Private Inference keys come from Audric](/private-inference)).
...
| `t2000_chat` · `models` — Audric [Private Inference](/private-inference), registered only when `T2000_API_KEY` is set | models |
