---
title: "Agent Wallet"
description: "The t2 CLI for AI agents on Sui — gasless USDC + USDsui sends, Cetus swaps, x402 paid API access, scriptable from any shell. Plus the @t2000/mcp server and 8 agent skills."
---
...
| `t2 chat <message>` · `t2 models` | Private inference on the [Private API](/private-api) (OpenAI-compatible; ZDR, a `phala/*` tier is GPU-TEE confidential). `t2 chat` streams; `--model` (default `zai/glm-5.2`), `--system`, `--no-stream`. Needs `T2000_API_KEY` (or `--api-key`) from platform.t2000.ai. `t2 models` lists the catalog. |
...
| MCP tools | 11 (7 read + 3 write + 1 settings) |
| Skills | 8 markdown playbooks shipped as `skill-<name>` MCP prompts |
