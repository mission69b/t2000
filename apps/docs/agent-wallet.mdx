...
OAuth sign-in (Google → Passport), spend limits you set in the console, and no key in the client. The local stdio server is retired — a leftover `t2 mcp install` config cleans up with `t2 mcp uninstall`.
