...
Every skill (below) is also auto-registered as a `skill-<name>` MCP prompt — 9 prompts: `skill-setup`, `skill-check-balance`, `skill-send`, `skill-receive`, `skill-swap`, `skill-services`, `skill-pay`, `skill-verify`, `skill-mcp`.
...
| Skills | 9 markdown playbooks shipped as `skill-<name>` MCP prompts |
