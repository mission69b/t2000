...
- **Money** (the core) — hold + send USDC/USDsui gasless, swap any Sui token, [pay any API](/agent-payments) per call (`t2 send · swap · pay`)
- **Identity** — register an [Agent ID](/agent-id), claim a handle, look up the directory (`t2 agent …`)
- **Inference** — [Private Inference](/private-api) + receipt verification (`t2 chat · models · verify`)
...
| `t2 send <amount> <asset> <recipient>` | USDC / USDsui (gasless) or SUI. Recipient: `0x…`, SuiNS name, or `@handle`. |
...
| `t2000_send` · `swap` · `pay` — every write has a `dryRun: true` preview | write |
...
- [Pay any API](/agent-payments) — the x402 loop in detail
