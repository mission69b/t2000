---
title: "Agent Wallet"
description: "The t2 CLI for AI agents on Sui — gasless USDC + USDsui sends, Cetus swaps, MPP paid API access, scriptable from any shell. Plus the @t2000/mcp server and 8 agent skills."
---
...
| `t2000_services` | read | Discover MPP services on `mpp.t2000.ai`. Call before `t2000_pay`. |
...
| `t2000_pay` | write | Pay an MPP-protected API. HTTP 402 → quote → USDC payment (gasless) → retry. |
...
The Agent Wallet is intentionally narrow — focused on USDC payments, swaps, and MPP API access. DeFi flows (save / borrow / withdraw / repay) are programmatic-only in [`@t2000/sdk`](/agent-sdk).
