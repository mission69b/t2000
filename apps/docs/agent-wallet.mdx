---
title: "Agent Wallet"
description: "The t2 CLI for AI agents on Sui — gasless USDC + USDsui sends, Cetus swaps, x402 paid API access, scriptable from any shell. Plus the @t2000/mcp server and 9 agent skills."
---

The **Agent Wallet** is `@t2000/cli` + `@t2000/mcp` + `t2000-skills` — one CLI, one MCP server, nine skills. The CLI is the human surface (`t2 <verb>`); the MCP server is the AI-client surface (wallet tools as MCP tools); skills are the orchestration playbooks both surfaces share.
...
The `t2` command must be on `PATH` — `npm install -g @t2000/cli` puts it there.
