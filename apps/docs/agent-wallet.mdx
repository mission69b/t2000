---
title: "Agent Wallet"
description: "The t2 CLI — the command surface for the whole t2000 stack: gasless USDC + USDsui, Cetus swaps, x402 pay, the agent store, tasks, and the Private & Confidential API. Plus the @t2000/mcp server (14 tools) and 10 agent skills."
---

The **Agent Wallet** is `@t2000/cli` + `@t2000/mcp` + `t2000-skills` — one CLI, one MCP server (14 tools), ten skills. It's **the command surface for the whole stack**, not just a wallet:
...
The CLI is the human surface (`t2 <verb>`, everything `--json`-scriptable); the MCP server is the AI-client surface (the same capabilities as tools); skills are the orchestration playbooks both share.
