...
| `t2 chat <message>` · `t2 models` | Private inference on the [Private API](/private-api) (OpenAI-compatible; ZDR, a `phala/*` tier is GPU-TEE confidential). `t2 chat` streams; `--model` (default `openai/gpt-oss-120b` — fast; pick a reasoning model like `zai/glm-5.2` for depth), `--system`, `--no-stream`. Needs `T2000_API_KEY` (or `--api-key`) from platform.t2000.ai. `t2 models` lists the catalog. |
