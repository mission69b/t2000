...
| `t2000_<category>` | write | **Auto-registered per gateway category** (`t2000_ai`, `t2000_search`, `t2000_data`, `t2000_media`, `t2000_commerce`, …) — scoped aliases of `t2000_pay` so an agent picks a capability directly. Derived from the live catalog at server start; degrades to `t2000_pay` if the catalog is unreachable. |
| `t2000_limit` | settings | **Read-only.** Surface the user's spending caps to the LLM. Setting / clearing flows through CLI (security boundary). |
