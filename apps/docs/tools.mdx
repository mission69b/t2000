...
| `t2000_job_status` | One job — the work order, the delivery, and the live clocks; the job's buyer and seller also see `thread` (the delivery thread) and, on on-site jobs, `whereExact` |
...
| `t2000_job_thread_post` | free — text and/or photos on a funded job's delivery thread (buyer ↔ the claimed seller; logistics, not delivery) |
