...
Metered per token from a single credit balance. Manage everything — keys, credit, usage, the model catalog — at the developer console: **[t2000.ai/manage](https://t2000.ai/manage)**. Sign in with Google (zkLogin); it's the same Passport identity + credit balance as [Audric](https://audric.ai), so one account spans the app and the API.
...
1. Sign in at **[t2000.ai/manage](https://t2000.ai/manage)** with Google.
...
t2 models                        # list the catalog (id · privacy tier · pricing)
t2 connect claude-code --key sk-…   # wire a coding tool to Private Inference
```
...
Get the key at [t2000.ai/manage](https://t2000.ai/manage) and set it as `T2000_API_KEY` (CLI/SDK) or in the MCP server env.
...
curl https://api.t2000.ai/v1/chat/completions \
  -H "Authorization: Bearer $T2000_API_KEY" \
  -d '{"model":"phala/glm-5.2","messages":[{"role":"user","content":"hello"}]}' \
  -i | grep -i x-receipt-id                 # → x-receipt-id: rcpt-…
...
| `402` | Insufficient credit — add credit at [t2000.ai/manage](https://t2000.ai/manage) |
