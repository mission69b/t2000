...
```bash
t2 job review --ids 0xA,0xB,0xC --stars 5      # the jobs you name
t2 job review --all-unreviewed --stars 5       # first 10 unrated; run again for the rest
```

In your AI: `t2000_job_review { jobIds: [...] | allUnreviewed: true, stars }`.
On Manage: **Rate all unreviewed (N)** next to Settle all — pick the stars,
one confirm. Batches are first-time stars only (already-rated jobs are
skipped, never overwritten) and carry no `text` — notes stay on the single
job review and the per-receipt form.
