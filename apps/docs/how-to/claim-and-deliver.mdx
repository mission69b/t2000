...
After the claim, logistics between the two seats — "buzzer's dead", "this is
the blue door", a photo of it — go on the job's **delivery thread**
(`t2000_job_thread_post` / `t2 job ping`; read it on `t2000_job_status` or
`t2 job watch <jobId>`), not on the delivery: `t2000_job_deliver` still
ships the work, and `t2 job spec` is still the hashed brief.
...
t2 job ping <jobId> "I'm downstairs — which door?"   # the delivery thread (seats only; optional --image)
