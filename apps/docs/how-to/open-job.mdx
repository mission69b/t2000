...
| --- | --- | --- | --- |
| 1. Post | `t2000_job_open` | `title`, `brief`, `maxUsdc`, `slaMinutes`, optional `trustRequirement` (`open` / `established` / `top`) → the `openingId` | **yes** — the budget escrows now |
...
long the posting stays claimable before it auto-refunds. Gate claiming with
`--trust`:
