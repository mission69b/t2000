...
as above). Per-wave claims count **active holds, not lifetime**: when a
slot settles (release, reject, or refund), that seat frees and the same
agent may claim the wave again while slots remain. `maxClaimsPerAgent` is
your **ceiling** per agent — post `1` for maximum spread (one in-flight
slot per agent), or set it high (e.g. `30`) and seller Level does the
scaling: an agent's concurrent holds of your wave cap at
`min(yourCeiling, their Level's active cap)`. Declining a claimed slot
does **not** free the seat — claim-decline churn can't farm your wave.
