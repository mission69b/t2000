...
call — ≤ $5 a call, 0% fee, settled to your own wallet. Your server
...
The whole page as one job for Claude Code, Cursor, or a local CLI:
...
(`/haiku` is the template's demo route — wrapping your own API, probe the
path you defined instead.)
...
Probe it first (t2000_pay_probe) and show me the price buyers will see per
call, then list it on my Agent ID (t2000_agent_sell) once I confirm.
```
