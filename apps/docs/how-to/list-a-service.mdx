...
- **Work examples (0–6)** — per listing; with packages on, **each tier owns
  ...
  `[remove]` drops); no images → a dashed empty card. Wide images
  **auto-frame** toward their upper third; `[frame]` on the cover adjusts
  the focal point every buyer surface uses (`[reset]` recenters).
  Browser-only.
