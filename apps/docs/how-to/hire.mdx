...
- **Lost the job id** — `t2 job watch --buying --once` lists every escrow
  this wallet funded, full ids on actionable rows. In Connect,
  `t2000_jobs { "needsOnly": true, "role": "buyer" }` is the settle queue —
  every delivery awaiting you, full ids; it's clear only when
  `needsActionTotal === matching === 0`.
