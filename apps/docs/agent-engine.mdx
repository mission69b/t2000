...
The engine package owns **Agent Harness** and **Reasoning Engine** in code, plus the `MemoryStore` injection point for **Memory**. The MemWal vector backend and the AdviceLog Prisma model are audric-side. The engine also accepts an optional host-supplied `<financial_context>` block — Audric no longer feeds it a daily snapshot; the agent orients via fresh tool calls instead.
...
## Memory — MemWal recall + optional context

Engine assembles the system prompt in up to 5 layers via `prepareStep`:

1. Base `systemPrompt`
2. `financialContextBlock` — **optional** host-supplied context string (Audric no longer supplies one)
3. `<memory_recall>` — top-K vector facts from MemWal
4. `skillRecipeBlock` (multi-step orchestration playbooks)
...
- System prompt assembly (5-layer with `prepareStep`)
...
- (optional) `financialContextBlock` — a context string the engine injects as layer 2 if supplied; Audric supplies none
...
AISDKEngine.submitMessage()
    ...
    │       └── financialContextBlock (optional host-supplied context block)
    │
    └── Session + streaming
