...
`McpClientManager` is backed by `@ai-sdk/mcp` (since engine v2.1.0). `McpPromptAdapter` is the (currently dormant) adapter that lets the engine consume MCP prompts as skill recipes — see `spec/reference/MCP_PROMPTS_INTEGRATION_DECISION.md` for the design rationale.
...
| Surface | Status |
|---|---|
| Major | 4.0.0 (version-locked with `@t2000/{sdk,cli,mcp}` — coordinated bump, no engine-side breaking changes) |
| Module format | ESM only (tsup → `dist/index.js`) |
| Tree-shake | Yes — all exports are named exports |
| Side effects | None |
