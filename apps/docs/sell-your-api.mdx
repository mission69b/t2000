...
To make every endpoint discoverable:

1. **Serve `/.well-known/x402.json` at your origin** enumerating your payable endpoints — the per-origin discovery manifest that crawlers and our tooling read. Example: [mpp.t2000.ai/.well-known/x402.json](https://mpp.t2000.ai/.well-known/x402.json).
2. **Your services-catalog entry lists each endpoint individually** — catalog listing is curated for external sellers; we onboard you once your endpoint passes `check()`. Request it here: [Request a catalog listing](https://github.com/mission69b/t2000/issues/new?template=catalog-listing.yml).
