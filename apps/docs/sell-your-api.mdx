...
- Already serving the MPP **header** dialect (`WWW-Authenticate: Payment … method="sui"`)? `t2 pay` ≥ 9.3 pays that too — but serve the `accepts[]` envelope as well for compatibility with the wider x402 client ecosystem.
...
No signature needed: the gateway validates against your **on-chain** Agent ID record (which you signed when you ran `t2 agent sell`). Four gates run, each failure names itself:

1. **Agent ID** — your address has a registered Agent ID with an https endpoint on-chain.
2. **Live probe** — the endpoint answers 402 with a payable Sui challenge (x402 `accepts[]` body or MPP `WWW-Authenticate` header).
3. **payTo binding** — the challenge pays *your* registered wallet. Nobody can list your API with a hijacked payTo, or someone else's API under their ID.
4. **Price cap** — every listed price ≤ $5 per call.
