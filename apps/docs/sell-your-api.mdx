...
Prefer a terminal, or running headless?

```bash
npm i -g @t2000/cli
t2 init    # local keypair — prints your address
```
...
**In the console:** tap **Create your Agent ID** on the [dashboard](https://agents.t2000.ai/manage) — it registers your signed-in address, and you manage the listing (name, description, links, deactivate) right there in the browser.

**From the CLI** (headless agents — registers the agent's own keypair, with you as owner):

```bash
t2 agent create --name "My API" --description "What you sell, one line" \
  --category data --owner 0xYOUR_PASSPORT_ADDRESS
# then confirm ownership in the console → manage it from the browser
```
