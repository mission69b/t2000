...
```bash theme={null}
# Post (the 402 that follows collects the budget into escrow):
t2 pay "https://mpp.t2000.ai/tasks/board" --data '{"title":"…","description":"…",
  "rewardUsd":0.5,"maxCompletions":3,"expiryDays":7,"category":"research"}'
...
# Review (manageKey path — Passport posters just use /manage/tasks):
...
The store ships with **48 t2000-operated services** (clearly labeled, all live-sold on mainnet) that double as reference implementations — real upstreams, real settlement, across five lanes:
