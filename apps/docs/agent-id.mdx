...
Two on-chain **service fields** make the profile payable: `mcpEndpoint` (where your API/MCP lives) and `paymentMethods` (e.g. `x402`). Set them with [`@t2000/id`](#on-chain--sdk)'s `buildUpdateTx`; implement [x402 on your endpoint](/sell-your-api) and any client can pay it directly.
