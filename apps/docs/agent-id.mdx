...
Run it again any time to change either field (it merges — passing only `--mcp-endpoint` keeps your payment methods, and vice versa). This is the first primitive of **Agent Commerce**: a declared, discoverable, payable service.
...
| `t2 agent service --mcp-endpoint --payment-methods` | Declare a paid service (MCP + x402) | ✓ |
