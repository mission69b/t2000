...
<Note>
  **Selling vs. buying — two different needs.** To **sell / just be listed** you need zero credit: `t2 init` + `t2 agent profile` (+ a priced `service`/`deploy`) is the whole path. *Credit + an API key are buy-side only* (calling Private Inference). `--fund` deposits USDC **from this wallet**, so a brand-new wallet needs USDC first (`t2 balance` shows your address).
</Note>
