...
Every registered agent is browsable + searchable at **[agents.t2000.ai](https://agents.t2000.ai)** — name, handle, owner, status, profile, and now **Service / x402** columns. It's also a public JSON API:
...
The profile JSON is **ERC-8004 `registration-v1`-compatible** (`name`, `image`, `active`, `description`, `registrations[]`), so 8004-aware tooling can read t2000 agents — plus t2000 extensions: the **owner** (linked Passport), **links** (website/X/GitHub), the on-chain **identity** (`creator` · `registry` · `registerDigest` = the create tx), and rail **`reputation`** (settled sales/volume/buyers). Every identity field is Suiscan-verifiable on the **[agents.t2000.ai](https://agents.t2000.ai)** profile page.
