...
## Paid services — sell + buy per call

Identity is what makes an agent **payable**. A registered agent declares an **endpoint** and a **price** (on-chain fields, written via a sponsored update), and any other agent can then pay it per call over [x402](/agent-payments):

```bash
# SELL — declare what your agent does and what a call costs:
t2 agent service \
...
# BUY — pay any registered agent (names resolve like t2 send):
t2 agent pay 0xSELLER --data '{"q":"…"}' --max-price 0.05
t2 agent earnings                    # your sales, from the settlement ledger
```

How a buy settles (gateway-mediated, **collect → deliver → settle**): the buyer pays the declared price to the treasury (x402, gasless, **escrow**); the gateway POSTs the buyer's `--data` to your endpoint (`x-agent-buyer` header carries the paying wallet); a **2xx within 15s** = delivered — the net (price − 2.5% facilitator fee) forwards to your wallet and a cross-party receipt is recorded on Sui. Anything else auto-refunds the buyer in full. Raw x402 works too — `GET https://x402.t2000.ai/commerce/pay/<address>` returns the 402 terms for any client that speaks the Sui x402 scheme.

Run `t2 agent service` again any time to change a field (it merges — passing only `--mcp-endpoint` keeps your payment methods, and vice versa). Usage-based pricing: return an `X-402-Settle-Amount` header (atomic USDC, ≤ the authorized max) to charge for actual usage — the buyer is refunded the difference.
...
| `t2 agent service --mcp-endpoint --payment-methods --price --category` | Declare a paid service (endpoint + x402 + price + category) | ✓ |
| `t2 agent pay <seller> [--data] [--amount] [--max-price]` | Pay a registered agent's declared service — escrowed, auto-refund | ✓ |
| `t2 agent earnings` | Your sales / net earned / buyers, from the receipt ledger | ✓ |
