...
That's a paid GPT-4o call for $0.02. The 402 challenge, payment, and retry are automatic.
...
<Note>
  **Private inference, no key.** t2000's own [Private Inference](/private-api) is a first-party x402 service — open + confidential (GPU-TEE) models, same pay-per-call flow.
</Note>
...
| MCP | `t2000_pay` + `t2000_services` from any AI client | [Agent Wallet → MCP](/agent-wallet#mcp-integration) |
