...
description: "Pay any API in USDC over the x402 rail. Every major AI + data API, no signup, no API keys — gasless USDC over Sui."
---

**Agent Payments** is the x402 rail — the gateway at [`mpp.t2000.ai`](https://mpp.t2000.ai) serves paid HTTP APIs that AI agents can pay for autonomously with USDC on Sui. No signup, no API keys, no accounts. Pay `$0.02 – $0.10` per LLM call. Pay `$2.00 – $3.00` per physical postcard.
...
Explore the whole rail before funding anything. Discovery + live metrics are open endpoints — no wallet, no payment, no account:
...
Every chat provider on the rail speaks the **OpenAI Chat Completions shape** — same request body, same response JSON. One request format, many providers, all **$0.02/call**, no signup and no API keys:

| Provider | Endpoint | Models |
|---|---|---|
| OpenAI | `/openai/v1/chat/completions` | GPT-4o, o1, GPT-4o-mini |
| Groq | `/groq/v1/chat/completions` | Llama 3, Mixtral, Gemma (ultra-fast) |
| DeepSeek | `/deepseek/v1/chat/completions` | DeepSeek-V3, R1 (reasoning) |
| Together | `/together/v1/chat/completions` | Llama 4, Qwen, Mixtral |
| Mistral | `/mistral/v1/chat/completions` | Mistral Large, Codestral |

OpenAI's other endpoints are mirrored too — `embeddings`, `images/generations`, `audio/speech`, `audio/transcriptions` — and more chat providers (Perplexity Sonar, Cohere, xAI) are one `t2 services search "chat"` away.
...
<Note>
  **Private inference on the rail.** t2000's own [Private API](/private-api) is a first-party x402 service too — `t2 pay https://x402.t2000.ai/t2000/v1/chat/completions` runs **open + confidential (GPU-TEE) models** with no key, no account. Same pay-per-call flow; private by default.
</Note>

### Calling it from code

The gateway answers a paid call with **HTTP 402 first**, then expects a signed `X-PAYMENT` retry — a handshake the stock OpenAI SDK can't perform on its own. So you call through a **t2000 paying client** (the CLI or the SDK), which does the 402 → pay → retry for you and hands back the **verbatim OpenAI response body**:
...
```ts
// SDK — result.body is the exact OpenAI Chat Completions JSON
import { T2000 } from '@t2000/sdk';

const agent = await T2000.create();
const { body } = await agent.pay({
  url: 'https://mpp.t2000.ai/groq/v1/chat/completions',
  method: 'POST',
  body: JSON.stringify({
...
<Note>
**Response shape = OpenAI; payment = the t2000 client.** The body you get back is byte-for-byte the OpenAI schema, so any OpenAI response parser works unchanged. The one thing the stock OpenAI SDK *can't* do is the x402 payment handshake (it has no hook to sign a Sui transaction on a 402) — route the call through `t2 pay` or `@t2000/sdk` `pay()` and you get the same JSON, paid per-call in USDC, no API key.
</Note>
...
- **Protocol** — the x402 rail (HTTP 402 payments on Sui), implemented via [`mppx`](https://www.npmjs.com/package/mppx) + [`@suimpp/mpp`](https://www.npmjs.com/package/@suimpp/mpp).
