...
- **Logging** — Prisma + NeonDB. Every payment is logged with its tx digest; browse at [`mpp.t2000.ai/activity`](https://mpp.t2000.ai/activity). Agent-to-agent purchases (Agent Commerce settlements) stream on the same feed.
