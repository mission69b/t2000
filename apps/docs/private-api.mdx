...
`phala/*` models run in a **verified GPU-TEE** and return a **signed receipt** you can anchor on Sui and **check yourself** with `t2 verify` — the only Sui-native *verifiable* confidential inference. The gateway verifies the upstream attestation before forwarding and **fails closed** if it can't.
