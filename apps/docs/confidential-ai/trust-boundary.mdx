...
Full no-trust closes the gap above: the gateway itself runs **in a TEE** (its own attestation), the receipt key lives in **dstack KMS**, and **end-to-end encryption** means even t2000 never sees plaintext. Plus local `dcap-qvl` quote re-verification on the client. This is the heaviest, most security-critical rung — built when a "can't-trust-the-gateway" need pulls for it.
