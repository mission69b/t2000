...
<Note>
  **What's trustless (honest):** the Sui anchor, the receipt signature, and the TDX quote are all verified **client-side** — the quote chains to Intel's root, so even the gateway's `verified:true` is no longer something you have to trust. The one remaining hop is `keyset_endorsement` (cryptographically tying the receipt-signing key to the quote's identity key); the receipt-signature check already proves the receipt is signed by the published keyset key. The gateway still terminates TLS and sees plaintext on its forwarding leg (ZDR) — full E2EE is the deferred v3.1 rung. See the [trust boundary](/confidential-ai/trust-boundary).
</Note>
