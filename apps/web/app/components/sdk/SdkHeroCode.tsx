...
const TABS = [
  ...
  { id: "api", label: "api.ts", sub: "Private inference" },
] as const;
...
const SAMPLES: Record<TabId, { file: string; lines: CodeToken[] }> = {
  ...
  payments: {
    ...
    lines: [
      ...
      { p: "new", c: " " },
      { type: "T2000", c: "();\n\n" },
      { co: "// hit any x402 endpoint — no API key\n" },
      { p: "const", c: " r = " },
      { p: "await", c: " t.pay({\n" },
      { c: "  url: " },
  ...
  api: {
    ...
    lines: [
      ...
      { p: "new", c: " " },
      { type: "T2000", c: "();\n\n" },
      { co: "// every model, one key, private by default\n" },
      { p: "const", c: " r = " },
      { p: "await", c: " t.chat({\n" },
      { c: "  model: " },
      ...
      { c: ",\n  messages,\n" },
      { c: "});\n\n" },
      { co: "// zero retention · confidential tier available\n" },
      { co: "// r.receipt → verify at verify.t2000.ai\n" },
      { p: "return", c: " r.text;" },
    ],
  },
};
