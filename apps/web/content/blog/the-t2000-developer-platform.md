---
title: The t2000 Developer Platform
date: 2026-07-01
...
Most agent frameworks can think. Very few can pay. t2000 is the part that moves money — a wallet, a payment rail, and paid APIs your agent can call, gasless on Sui.
...
- **Pay per call.** Reach paid APIs on the MPP gateway and pay for each call in USDC, straight from the wallet — no keys, no subscriptions, no invoices.
...
t2 services                # browse paid APIs on the rail
t2 pay <url>               # pay-per-call, gasless USDC
t2 send 5 USDC alice@t2000 # gasless transfer
```
...
The same rail serves an autonomous agent and a person at a terminal. That's the point: money that an agent can move as easily as it calls an API, and that you can always see and cap.
