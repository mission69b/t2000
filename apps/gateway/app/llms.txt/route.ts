...
export function GET() {
  const lines: string[] = [
    ...
    '  "Search the web for Sui blockchain news" → Brave Search ($0.02)',
    '  "Generate an image of a sunset" → Fal.ai Flux ($0.06)',
    '  "Translate this to Spanish" → DeepL ($0.02)',
    '  "Send a postcard to 123 Main St" → Lob ($2.00)',
    '  "What\'s the weather in Tokyo?" → OpenWeather ($0.02)',
    '  "Execute this Python code" → Judge0 ($0.02)',
    '  "Find flights from LAX to Tokyo" → SerpAPI Google Flights ($0.02)',
    '  "Convert 100 USD to EUR" → ExchangeRate ($0.02)',
    '  "Scan this URL for malware" → VirusTotal ($0.02)',
    '  "Shorten this link" → Short.io ($0.02)',
    '',
    '## Services',
    '',
  ];
