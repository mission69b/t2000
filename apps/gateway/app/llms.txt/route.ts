...
export function GET() {
  const lines: string[] = [
    ...
    '  "Search the web for Sui blockchain news" → Brave Search ($0.012)',
    '  "Generate an image of a sunset" → Fal.ai Flux ($0.036)',
    '  "Translate this to Spanish" → DeepL ($0.012)',
    '  "Send a postcard to 123 Main St" → Lob ($1.20)',
    '  "What\'s the weather in Tokyo?" → OpenWeather ($0.012)',
    '  "Execute this Python code" → Judge0 ($0.012)',
    '  "Find flights from LAX to Tokyo" → SerpAPI Google Flights ($0.012)',
    '  "Convert 100 USD to EUR" → ExchangeRate ($0.012)',
    '  "Scan this URL for malware" → VirusTotal ($0.012)',
    '  "Shorten this link" → Short.io ($0.012)',
    '',
    '## Services',
    '',
  ];
