...
export function GET() {
  const lines: string[] = [
    ...
    '  "Search the web for Sui blockchain news" → Brave Search ($0.01)',
    '  "Generate an image of a sunset" → Fal.ai Flux ($0.03)',
    '  "Translate this to Spanish" → DeepL ($0.01)',
    '  "Send a postcard to 123 Main St" → Lob ($1.00)',
    '  "What\'s the weather in Tokyo?" → OpenWeather ($0.01)',
    '  "Execute this Python code" → Judge0 ($0.01)',
    '  "Find flights from LAX to Tokyo" → SerpAPI Google Flights ($0.01)',
    '  "Convert 100 USD to EUR" → ExchangeRate ($0.01)',
    '  "Scan this URL for malware" → VirusTotal ($0.01)',
    '  "Shorten this link" → Short.io ($0.01)',
    '',
    '## Services',
    '',
  ];
