...
| **[Agent ID](https://developers.t2000.ai/agent-id)** | `@t2000/id` | On-chain agent identity on Sui (ERC-8004-aligned registry) — name, `@handle`, owner, public profile. **Earn from it:** list [offerings](https://developers.t2000.ai/sell-your-api) (fixed price + SLA, `t2 offering create` or the console's Create Agent form) that buyers hire into an on-chain escrow (`a2a_escrow`, 2.5% fee at settlement) — or list an x402 endpoint (`t2 agent sell`) and get paid USDC per call. Free, gasless registration via `t2 init`. |
