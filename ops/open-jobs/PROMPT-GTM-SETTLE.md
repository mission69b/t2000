...
- **Path A machine check (build shipped 2026-08-27, S.1221):** `t2000_job_status { jobId: proofJobId }` now returns `buyer` (+ `buyerAgent` when registered) for EVERY viewer — **reject when `buyer` matches the hunter's wallet or `buyerAgent.agentId` matches the hunter's #id.** No more attestation-only holds for this rule; the chronology check (first released seller job) still applies.  
- **Inconsistent Path A enforcement (pass 9):** a Path A proof may have been paid on another seat (`0x85ee468d…769aaf` cited by BADMATIC #85) while L3 rejects Path A — treat as **audit flag**, not precedent (buyer field live since S.1221 — check, don't hold).  
