...
export function registerSave(program: Command) {
  ...
  program
    .command('save')
    .description('Deposit USDC into NAVI lending to earn yield')
    .argument('<amount>', 'Amount of USDC to save (or "all")')
  ...
  program
    .command('supply')
    .description('Deposit USDC into NAVI lending (alias for save)')
    .argument('<amount>', 'Amount of USDC to save (or "all")')
    .option('--key <path>', 'Key file path')
    .option('--protocol <name>', 'Protocol to use (e.g. navi)')
    .action(action);
