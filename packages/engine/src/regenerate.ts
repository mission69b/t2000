...
export async function regenerateBundle(
  ...
  // Pre-v0.7 this short-circuited on `!action.steps || steps.length < 2`
  // to reject single-write actions. Lifted because the engine now
  // populates `canRegenerate` + `regenerateInput` on single-write
  // confirm-tier actions too (see engine.ts single-write yield
  // path), and the rebuild branch below handles N=1 symmetrically.
  // Function keeps the name `regenerateBundle` for backward-compat
  // with hosts; a future major can rename to `regenerateAction`.
  ...
  // Reconstruct the writes the original action carried. The rebuild
  // branches on shape: bundle (N≥2 with `steps`) goes through
  // `composeBundleFromToolResults`; single-write (N=1) builds the
  // legacy shape directly + re-uses `computeRegenerateFields` so the
  // freshness fields stay in lockstep with the chat-time emission
  // path (see engine.ts single-write yield).
  ...
  if (action.steps && action.steps.length >= 2) {
    // Bundle path — re-stamp every step's attemptId via the bundle
    // composer.
    ...
    // [SPEC 15 v0.7 follow-up] Single-write rebuild. Mirror the
    // engine.ts single-write yield shape: fresh attemptId (per
    // SPEC 1 — host writes a new TurnMetrics row keyed on it),
    ...
    newPendingAction = {
      ...
      assistantContent,
      completedResults,
      ...(action.guardInjections?.length
        ? { guardInjections: action.guardInjections }
        : {}),
