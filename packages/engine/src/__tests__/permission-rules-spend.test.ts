...
describe('resolvePermissionTier — sessionSpendUsd cumulative cap', () => {
  it('returns auto when sessionSpendUsd is undefined (current behavior preserved)', () => {
    expect(resolvePermissionTier('repay', 40, DEFAULT_PERMISSION_CONFIG)).toBe('auto');
    expect(resolvePermissionTier('repay', 40, DEFAULT_PERMISSION_CONFIG, undefined)).toBe('auto');
  });

  it('returns auto when cumulative spend stays under the daily limit', () => {
    // 100 + 40 = 140, well under 200 daily limit
    expect(resolvePermissionTier('repay', 40, DEFAULT_PERMISSION_CONFIG, 100)).toBe('auto');
  });
  ...
  it('downgrades auto to confirm when cumulative spend would breach the daily limit', () => {
    ...
    expect(resolvePermissionTier('repay', 40, DEFAULT_PERMISSION_CONFIG, 180)).toBe('confirm');
  });
  ...
  it('does not weaken tiers — confirm/explicit pass through unchanged regardless of spend', () => {
    ...
    expect(resolvePermissionTier('repay', 100, DEFAULT_PERMISSION_CONFIG, 0)).toBe('confirm');
    expect(resolvePermissionTier('repay', 100, DEFAULT_PERMISSION_CONFIG, 1000)).toBe('confirm');
    // 1500 USD save is explicit; spend never elevates that to auto.
    expect(resolvePermissionTier('repay', 1500, DEFAULT_PERMISSION_CONFIG, 0)).toBe('explicit');
    expect(resolvePermissionTier('repay', 1500, DEFAULT_PERMISSION_CONFIG, 5000)).toBe('explicit');
  });
  ...
  it('treats spend exactly equal to the daily limit as still autonomous', () => {
    ...
    expect(resolvePermissionTier('repay', 40, DEFAULT_PERMISSION_CONFIG, 160)).toBe('auto');
  });
});
