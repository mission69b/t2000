...
> **Last refreshed:** 2026-05-24 ~12:30 AEST after S.291 — AI SDK Hardening **P5.1** (edit user message + re-send + `truncateMessagesAfter` + 11 chat-persistence tests) SHIPPED (audric `9029eb1`). Phase 5 of `SPEC_AI_SDK_HARDENING.md` now has just **P5.6 remaining**. Tracked-file count unchanged (still 4 shipping files). Earlier today: S.290 (P5.2 + P5.3 + P5.4 batch), SPEC promotion (`.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` → `spec/active/shipping/SPEC_AI_SDK_HARDENING.md`), S.289 Phase 3 self-audit fixes (audric `2239d30`, t2000 `eb9c0e21`), S.288 (Phase 2 + flag drop), S.287 (Phase 1 ship + SPEC promotion `spec/active/` → `shipping/`), S.286 (SPEC v0.1 → v0.2), S.285 (AI SDK Hardening Phase 2 code ship). Yesterday (2026-05-23): S.283 closed PIPELINE-AUDIT-PHASE-2 S5 as no-op; S.282 (S3 canvases), S.281 (S2 rename), S.280 (S1 split, patch v2.19.2).
...
| `SPEC_AI_SDK_HARDENING.md` | 🟢 SHIPPING (NEW 2026-05-24) | Promoted today from `.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` so the next session can pick up where the last left off without re-litigating completed phases. **Phase 1 SHIPPED** (S.284). **Phase 2 SHIPPED** (S.285 + S.286-S.289 for P2.2 / Stream Resume sub-SPEC). **Phase 5 mostly SHIPPED** (S.290 — P5.2+P5.3+P5.4; S.291 — P5.1 edit/truncate/5 tests; P5.5 via Stream Resume). **Only P5.6 remains in Phase 5** (engine metadata HF/APY/per-reward). Phases 6/3/7/4 pending. Promote to `archive/v07f/` once all 7 phases marked SHIPPED or cleanly deferred. |
