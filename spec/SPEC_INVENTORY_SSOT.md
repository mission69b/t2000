...
| 🚀 Shipping (first phase shipped, follow-ups open) | **3** | In `spec/active/shipping/` (SPEC 30, SPEC 272, SPEC_AUDRIC_STREAM_RESUME) |
...
### 1.1.d — additional archives 2026-05-25 (post-P4.1 close)
...
### 1.2 `spec/active/shipping/` (8 files — SMOKE-2 trio added 2026-05-25)
...
| `SPEC_AI_SDK_HARDENING.md` | 🟢 SHIPPING — 6/7 phases SHIPPED, Phase 7 4/5 SHIPPED (P7.5 only remainder) | Phase 1 SHIPPED (S.284). Phase 2 SHIPPED (S.285 + S.286-S.289 for P2.2 / Stream Resume sub-SPEC). Phase 5 CLOSED (S.290 — P5.2+P5.3+P5.4; S.291 — P5.1; S.292 — P5.6; P5.5 via Stream Resume). Phase 6 SHIPPED (S.298 typed-error classification + S.299 PII self-audit). Phase 3 CLOSED (S.300 P3.2 + S.301 P3.4 + S.302 P3.1 + S.303 P3.3 dormant seam decision doc + S.309 P3.1 smoke hotfix). Phase 4 CLOSED (S.304 P4.7 + S.305-S.308 decision docs + S.311 P4.3 DEFERRED + **P4.1 SHIPPED 2026-05-25 as engine v3.0.0** + **P4.1 follow-up SHIPPED 2026-05-25 as engine v3.1.0** — dead `LLMProvider` pathway retired, CLI/MCP ESLint configs landed, Audric `TurnMetrics.todoUpdateCount` dropped via Prisma migration). **Phase 7 — P7.2 + P7.3 SHIPPED 2026-05-25 (S.314)** — engine compose-bundle invariant tests (+5) + audric `apps/web-v2` plumbing for `inputCoinFromStep` + `cetusRoute` end-to-end through marker → client → sponsored-tx → prepare-route → SDK `composeTx`. **Only P7.5 (streaming bundle assembly progress UX) remains.** Promote to `archive/v07f/` once P7.5 marked SHIPPED or cleanly deferred. |
| `SPEC_AI_SDK_HARDENING_V3_SMOKE.md` | 🟢 ACTIVE (NEW 2026-05-25) | v3.0.0 / v3.1.0 + Phase 7 P7.2/P7.3 (S.314) prod smoke checklist for audric. 6 items (V3-SMOKE-1 through V3-SMOKE-6) — save / borrow / swap / `update_todo` regression / chain-mode bundle / read tools sample. Templated on `archive/v07e/SPEC_AI_SDK_HARDENING_PHASE_5_SMOKE.md`. Founder runs ~12-15 min on `audric.ai`. Archive to `archive/v07f/` after smoke passes. |
