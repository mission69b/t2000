...
## 0. TL;DR — current state (post-S.327 — Agent Wallet Greenfield Pivot SPEC drafted; Phase A pending)

S.327 added 4 files (1 anchor SPEC in `shipping/` + 3 deferred stubs in `active/`) + marked SPEC_AGENTIC_STACK Phase 5 ABSORBED. `spec/active/` (top level) now holds **8 working files** + 2 subdirs; `spec/active/shipping/` holds **8 files** (Agentic Stack stays in shipping until pivot ships then promotes to archive).
...
| 🚀 Shipping (active or recently-shipped multi-phase SPECs) | **8** | In `spec/active/shipping/` — Agent Wallet Greenfield is the anchor; Agentic Stack closes when pivot ships |
...
| 7 | `SPEC_MARKETING_SITE_REDESIGN.md` | 🟡 DEFERRED-STUB (NEW S.327) | `t2000/apps/web` marketing site redesign — homepage prompts panel, MPP catalog browser, brand assets. NOT `developers.t2000.ai` (owned by pivot Phase F via Mintlify). |
...
### 1.2 `spec/active/shipping/` (8 files — `SPEC_AGENT_WALLET_GREENFIELD.md` added in S.327)

| File | Status | Action |
|---|---|---|
| `SPEC_AGENT_WALLET_GREENFIELD.md` | 🟢 SHIPPING (NEW S.327, Phase A pending) | **The anchor SPEC for the next ~9-10 days.** CLI greenfield rewrite + brand restructure to AFI/4-sub-product + gRPC for gasless send/pay + USDC+USDsui-only + PIN removed + default safeguards removed + `developers.t2000.ai` via Mintlify. 7 phases (A-G). Ships as `@t2000/{sdk,cli,mcp}@4.0.0` major; engine 4.0.0 version-locked (no breaking changes). Detailed file-by-file plan at `.cursor/plans/agentic_wallet_pivot_spec_111d2729.plan.md`. |
| `SPEC_AGENTIC_STACK.md` | ✅ EFFECTIVELY CLOSED (S.327) | Phase 1 SHIPPED S.321, Phase 2+3 SHIPPED S.322, Phase 4 SHIPPED S.326. **Phase 5 ABSORBED 2026-05-26 into `SPEC_AGENT_WALLET_GREENFIELD.md` Phase D** (the marketing+README sweep folds into the larger brand restructure work — running Phase 5 first would just be wasted work that the pivot's Phase D would rewrite hours later). Promote this SPEC to `spec/archive/v07f/` once the pivot ships. |
...
| `SPEC_CLI_v3_SMOKE.md` | 🟢 ACTIVE (NEW 2026-05-25) | SMOKE-2 slice for `@t2000/cli@3.1.0`. 5 items — `init` happy path, `balance`, self-send 0.5 USDC, `save 1 USDC`, `rates`. ~6-8 min, ~$0.05 in gas. Archive after PASSED. |
| `SPEC_MCP_v3_SMOKE.md` | 🟢 ACTIVE (NEW 2026-05-25) | SMOKE-2 slice for `@t2000/mcp@3.1.0`. 5 items — server starts + tools registered, `t2000_overview`, `t2000_rates`, `t2000_send` with safeguard, deleted-tools absence check. ~5-7 min via Claude Desktop or Cursor. Archive after PASSED. |
