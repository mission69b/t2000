...
## 0. TL;DR — current state (post-S.343 — Phase F SHIPPED; pivot at 6/7 phases; pending = Phase G publish only)

S.342 archived 4 SPECs into the new `spec/archive/v4-greenfield/` directory: 3 v3-smoke checklists (CLI/MCP/SDK) + `SPEC_AGENTIC_STACK.md` (Phase 5 absorbed into pivot, fully shipped via S.341). `spec/active/shipping/` is now down to 4 SPECs.
...
| 🚀 Shipping (active or recently-shipped multi-phase SPECs) | **4** | In `spec/active/shipping/` — `SPEC_AGENT_WALLET_GREENFIELD.md` is the pivot anchor (~2 phases left); `SPEC_30` Phase 2-10 + `SPEC_272` Lever 2-3 + `SPEC_AUDRIC_STREAM_RESUME` Phase 4 all have open follow-ups |
...
| 📦 Archived (2026-05-26 / S.342 — v4 greenfield pivot Phase E) | **4** | Moved into new `spec/archive/v4-greenfield/` — see §1.1.e |
...
### 1.2 `spec/active/shipping/` (4 files — `SPEC_AGENT_WALLET_GREENFIELD.md` added in S.327, 4 archived in S.342)

| File | Status | Action |
|---|---|---|
| `SPEC_AGENT_WALLET_GREENFIELD.md` | 🟢 SHIPPING (S.327 drafted; Phases A-E SHIPPED 2026-05-26 across S.328-S.342; Phases F-G pending) | **The anchor SPEC for the v4 pivot.** A (CLI greenfield + gRPC/gasless, S.328-S.335) + B (skills + MCP sweep, S.336+S.339+S.340) + C (`t2` bin, S.340) + D (3 package READMEs + apps/web redeploy, S.341) + E (test + spec + RPC cleanup, S.342) all ✅ DONE. Pending: F (Mintlify at `developers.t2000.ai`, ~1d) + G (publish `@t2000/{sdk,cli,mcp}@4.0.0` + bump engine 4.0.0 version-locked, ~0.5d). Ships as the v4 major release once Phase G triggers. Promote to `spec/archive/v4-greenfield/` once Phase G publishes. |
...
### 1.2.a `spec/archive/v4-greenfield/` — NEW 2026-05-26 (S.342)

New archive subdirectory created in Phase E to hold the v4 Agent Wallet Greenfield Pivot's archived collateral. Holds 4 files moved in S.342:
