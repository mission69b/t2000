...
| `SPEC_AI_SDK_HARDENING.md` | 🟢 SHIPPING (NEW 2026-05-24) | Promoted today from `.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` so the next session can pick up where the last left off without re-litigating completed phases. **Phase 1 SHIPPED** (S.284). **Phase 2 SHIPPED** (S.285 + S.286-S.289 for P2.2 / Stream Resume sub-SPEC). **Phase 5 CLOSED** (S.290 — P5.2+P5.3+P5.4; S.291 — P5.1 edit/truncate/5 tests; S.292 — P5.6 live HF/APY metadata; P5.5 via Stream Resume). **Phase 6 SHIPPED** (S.298 typed-error classification + S.299 PII self-audit). **Phase 3 CLOSED** (S.300 P3.2 + S.301 P3.4 + S.302 P3.1 + S.303 P3.3 dormant seam decision doc + S.309 P3.1 smoke hotfix). **Phase 4 IN PROGRESS** — 6/7 closed: S.304 (P4.7 USD-aware rule correction) + S.305-S.308 (4 decision docs: `CANVAS_VS_ARTIFACT.md` / `PRISMA_VS_DRIZZLE.md` / `LONG_RUNNING_WORKFLOWS.md` / `LLM_CACHING_DECISION.md`) + S.311 (P4.3 DEFERRED via `SUBAGENT_PILOT_DEFERRAL.md`). Only P4.1 (engine v3.0.0 defineTool→tool migration) + Phase 7 (bundle hardening remainder) open. Promote to `archive/v07f/` once all 7 phases marked SHIPPED or cleanly deferred. |
