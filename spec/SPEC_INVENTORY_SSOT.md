...
| File | Status | Action |
|---|---|---|
| `SPEC_AGENTIC_STACK.md` | 🟢 SHIPPING (3/5 phases shipped) | Phase 1 SHIPPED S.321 (`@t2000/*@3.2.0` — CLI/SDK Tier 1: 4 SDK fixes + 6 CLI fixes + 24 new tests). **Phase 2+3 SHIPPED S.322** (bundled, no npm release — skills + routes are static): 4 new skills (`t2000-setup` / `t2000-swap` / `t2000-stake` / `t2000-yields`), Rules blocks on 5 write skills (`save`/`borrow`/`send`/`repay`/`withdraw`), `t2000-engine` description drift fix, Next.js dynamic route at `apps/web/app/skills/[slug]/route.ts` + `.well-known/agent-skills/index.json` manifest via `outputFileTracingIncludes` (glob key `/skills/*`, NOT `/skills/[slug]` — picomatch character-class trap), 18-skill manifest verified, one-prompt install canonical = `curl -sL https://t2000.ai/skills/t2000-setup`. **Phase 4** (MPP recipes — 3-4h) + **Phase 5** (Marketing + README sweep — ½d) pending. Locked decisions intact: hosting at `t2000.ai/skills/*`, HTTP MCP DEFERRED to standalone SPEC, gateway simplification DEFERRED to `SPEC_GATEWAY_SIMPLIFICATION`. |
