...
> **Last refreshed:** 2026-05-23 ~21:30 AEST after S.282 (PIPELINE-AUDIT-PHASE-2 S3 — 2 audric canvases migrated to the shared `usePortfolio` SWR cache; audit's "8 canvases" claim was wrong, ground-truth showed only 2). `spec/active/AUDIT_ON_CHAIN_PIPELINE_2026-05-23.md` ship log + recommendation matrix updated in place — S1 + S2 + S3 ✅ shipped (only S5 pending, gated on §7.1 telemetry). Audric-only commit; no engine bump. The audit doc is gitignored (founder-local), so tracked file counts unchanged. Earlier today: S.281 (S2 rename), S.280 (S1 split, patch v2.19.2). Last `spec/` tracked-file mutation was S.278 (SPEC 272 Lever 1 moved to `active/shipping/`).
