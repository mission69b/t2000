...
> **Last refreshed:** 2026-05-24 ~12:00 AEST after S.290 — AI SDK Hardening Phase 5 batch (P5.2 Copy + P5.3 Vote hydration + P5.4 Regenerate) SHIPPED (audric `313f6ed`). `SPEC_AI_SDK_HARDENING.md` updated to mark the three P-items SHIPPED. No tracked-file count change vs. ~11:40 (still 4 shipping files; the SPEC promotion happened earlier in the same session). Earlier today: SPEC promotion (`.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` → `spec/active/shipping/SPEC_AI_SDK_HARDENING.md`), S.289 Phase 3 self-audit fixes (audric `2239d30`, t2000 `eb9c0e21`), S.288 (Phase 2 + flag drop), S.287 (Phase 1 ship + SPEC promotion `spec/active/` → `shipping/`), S.286 (SPEC v0.1 → v0.2), S.285 (AI SDK Hardening Phase 2 code ship). Yesterday (2026-05-23): S.283 closed PIPELINE-AUDIT-PHASE-2 S5 as no-op; S.282 (S3 canvases), S.281 (S2 rename), S.280 (S1 split, patch v2.19.2).
...
| `SPEC_AI_SDK_HARDENING.md` | 🟢 SHIPPING (NEW 2026-05-24) | Promoted today from `.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` so the next session can pick up where the last left off without re-litigating completed phases. **Phase 1 SHIPPED** (S.284). **Phase 2 SHIPPED** (S.285 + S.286-S.289 for P2.2 / Stream Resume sub-SPEC). **Phase 5 batch SHIPPED** (S.290 — P5.2 Copy + P5.3 Vote hydration + P5.4 Regenerate). P5.5 already via Stream Resume. **P5.1 + P5.6 remain** as separate ~1d PRs. Phases 6/3/7/4 pending. Promote to `archive/v07f/` once all 7 phases marked SHIPPED or cleanly deferred. |
