...
### 1.2 `spec/active/shipping/` (4 files)
...
| `SPEC_AI_SDK_HARDENING.md` | 🟢 SHIPPING (NEW 2026-05-24) | Promoted today from `.cursor/plans/ai_sdk_hardening_bc37c5e8.plan.md` so the next session can pick up where the last left off without re-litigating completed phases. **Phase 1 SHIPPED** (S.284). **Phase 2 SHIPPED** (S.285 + S.286-S.289 for P2.2 / Stream Resume sub-SPEC). **Phase 5 CLOSED** (S.290 — P5.2+P5.3+P5.4; S.291 — P5.1 edit/truncate/5 tests; S.292 — P5.6 live HF/APY metadata; P5.5 via Stream Resume). **Phase 6 SHIPPED** (S.298 typed-error classification + S.299 PII self-audit). **Phase 3 CLOSED** (S.300 P3.2 + S.301 P3.4 + S.302 P3.1 + S.303 P3.3 dormant seam decision doc at `spec/reference/MCP_PROMPTS_INTEGRATION_DECISION.md`). **Phase 4 IN PROGRESS** — 5/7 closed: S.304 (P4.7 USD-aware rule correction) + S.305-S.308 (4 decision docs: `CANVAS_VS_ARTIFACT.md` / `PRISMA_VS_DRIZZLE.md` / `LONG_RUNNING_WORKFLOWS.md` / `LLM_CACHING_DECISION.md`). Only P4.1 (engine v3.0.0 defineTool→tool migration) + P4.3 (subagent pilot) + Phase 7 (bundle hardening remainder) open. Promote to `archive/v07f/` once all 7 phases marked SHIPPED or cleanly deferred. |
...
### 1.5 `spec/reference/` + `spec/runbooks/` (21 files — +4 in S.305-S.308)

All long-lived, all current. No drift. No action. S.305-S.308 (2026-05-24) batched **4 new decision docs under `spec/reference/`** as part of the AI SDK Hardening Phase 4 doc work:
