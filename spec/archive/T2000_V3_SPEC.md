...
**Created:** March 2026
**Status:** ✅ Shipped — Architecture Reference (app.t2000.ai live)
...
| **Multi-chain?** | **No. Sui only.** Earn the right to go multi-chain by being the best on one chain. Don't chase OWS/multi-chain until the Sui product is rock solid (see `spec/archive/OWS_INTEGRATION_SPEC.md`). |
| **OWS integration?** | **Not now.** OWS is 10 days old. We've already built what we need (key management, signing, safeguards). `TransactionSigner` makes OWS a future option with no current cost. Watch, don't adopt. |
...
#### Balance Header — Expandable (tap to reveal)

The balance header is always visible at the top. **Tapping the balance area toggles an expandable breakdown** — no need to use the Report chip just to see your account details. This mirrors how banking apps (Revolut, Monzo) show account info: always accessible, never buried in a feed.
...
When the user has active debt, the expanded view also shows:
- Debt amount (amber/warning color)
- Health Factor (amber when < 1.5)

**Design rationale:** The compressed one-liner (`chk $105 · sav $880`) gives the 2-second glance. The expansion gives the 10-second deep look. Neither pollutes the conversational feed.

#### First-Open-of-Day Report

On the **first dashboard open each day**, an account summary report card is automatically injected into the feed. This gives users a "morning briefing" alongside overnight earnings without requiring any action.
...
This only appears once per day (tracked via localStorage). Subsequent visits show smart cards only — no repeated summaries.
...
| **← Received funds** | Incoming transfer detected | "Received $500 from 0x7f..." | [Save it] [Swap] | `t2000_balance` polling | MVP |
...
```
[Save] [Send] [Pay] [Borrow] [Withdraw] [Repay] [Swap] [Receive] [Report] [History] [Help]
```

**[More...]** expands to:

```
[Borrow] [Withdraw] [Repay] [Swap]
[Receive] [Report] [History] [Help]
[Help]
```
...
**[Swap]** → asset grid → amount sub-chips → quote → confirm
...
- When funds arrive (detected via polling), a proactive card appears: "You received $1,000! [Save it] [Swap] [Keep it]" — actionable chips, no typing.
...
**Swap** — via [Swap] → [SUI] → [USDC] → [$200] chips, or typed "buy $200 SUI":
...
| Three value props (earn, pay, swap) | ✅ | "Your money earns 6-8%", "Pay for any service", "Buy, sell, and swap crypto with one tap" |
...
#### Dashboard — Smart Cards (Updated Mar 2026)

The dashboard's three-zone layout (balance top, cards middle, input bottom) is correct. **Balance header is now expandable** — tap to reveal full account breakdown (savings, APY, assets, debt+HF). First-open-of-day auto-injects a "Good morning" report card into the feed.

| Smart card | Spec trigger | Implementation status |
|-----------|-------------|---------------------|
| 🏆 Rewards | `pendingRewards > 0` | ✅ Live — claims via NAVI sponsored tx |
| 💰 Idle funds | `checking > $5` + `savingsRate > 0` | ✅ Live — shows rate + monthly earnings |
| 📈 Better rate | `bestAlternativeRate.rate - currentRate > 0.3%` | ✅ Live — shows protocol comparison + rebalance guidance |
| 💵 Overnight earnings | `isFirstOpenToday` + `overnightEarnings > 0` | ✅ Live — localStorage delta tracking |
| ← Received funds | Balance increase between polls | ✅ Working |
| ⚠ Risk | `healthFactor < 1.5` | ✅ Live — NAVI health data |
| ⚠ Session expiry | `sessionExpiringSoon` | ✅ Working |

**Result:** All 7 MVP smart cards functional with live data from NAVI Protocol and Cetus DEX.

#### Chip Flows — Functional with Context

| Flow | Spec context message | Current message |
|------|---------------------|----------------|
| Save | "Save to earn 6.8%. You have $105 available." | ✅ "Save to earn 3.9%. You have $105 available." |
| Send | "Who do you want to send to?" + contact chips + [📋 Paste] | ✅ Contacts + address input + Paste button + Go button |
| Withdraw | "Withdraw from savings. You have $880 saved." | ✅ "Withdraw from savings. You have $880 saved." |
| Borrow | "Borrow against your savings. You can borrow up to $440." | ✅ "Borrow against your savings. You can borrow up to $440." |
| Repay | Only when debt > 0 | ✅ Validates active debt, shows informative message if none |
| Swap | Asset grid → amount → quote → confirm | ✅ Full flow with Cetus quotes, buy/sell/swap labeling |
...
1. **Chip bar** — All chips are now visible in a wrapping row: [Save] [Send] [Pay] [Borrow] [Withdraw] [Repay] [Swap] [Receive] [Report] [History] [Help]. No "More..." menu needed. Active flow chip is highlighted.
...
### Multi-Protocol DeFi Support — Gap Analysis (Feb 2026)

**Status:** The SDK supports NAVI + Suilend + Cetus via `ProtocolRegistry`, but the web-app API routes only wire **NAVI** for lending and **Cetus** for swaps. Suilend is not used for rates, positions, or transactions. The `rebalance()` SDK method (atomic withdraw → swap → re-deposit to best yield) is not exposed via any web-app route.

This means:
- `/api/rates` returns NAVI rates only — `bestSaveRate` is always a NAVI rate
- `/api/positions` returns NAVI positions only — users with Suilend positions won't see them
- "Better rate" smart card compares NAVI vs NAVI (never surfaces a real Suilend alternative)
- The [Switch] action on the better rate card shows manual guidance text, not an atomic rebalance
- The agent's `get_rates` tool returns NAVI-only data despite the description saying "NAVI, Suilend, etc."

**What the SDK provides (already built, just not wired):**

| SDK Feature | Method | Notes |
|-------------|--------|-------|
| Multi-protocol rate comparison | `ProtocolRegistry.allRates(asset)`, `allRatesAcrossAssets()` | Aggregates rates from all registered `LendingAdapter`s |
| Best rate discovery | `ProtocolRegistry.bestSaveRate(asset)`, `bestSaveRateAcrossAssets()` | Returns best across NAVI + Suilend |
| Multi-protocol positions | `ProtocolRegistry.allPositions(address)` | Merges supplies/borrows from all protocols |
| Atomic rebalance | `T2000.rebalance()` | Withdraw from worst-APY → swap if cross-asset → deposit to best-APY. Single PTB when `addWithdrawToTx`/`addSaveToTx` supported. |
| Suilend lending | `SuilendAdapter` | Full `LendingAdapter`: save, withdraw, borrow, repay, getRates, getPositions, getHealth, getPendingRewards |
| Cross-asset swaps | `CetusAdapter` with stablecoin pairs | Used by rebalance when source/target assets differ |

**Implementation plan:** See `V3_BUILD_PLAN.md` Phase POL-13 for concrete tasks.

**Guiding principles:**
1. **Use `ProtocolRegistry`** — don't duplicate rate-comparison or position-aggregation logic in the web-app. The SDK's registry already handles multi-protocol aggregation correctly.
2. **Protocol-aware transactions** — `/api/transactions/prepare` must accept a `protocol` field to route save/withdraw/borrow/repay to the correct adapter (NAVI or Suilend).
3. **Rebalance as a first-class transaction type** — add `rebalance` to the prepare route's `TxType` union. The SDK's composed-PTB logic (withdraw → swap → deposit in one tx) should be used when possible.
4. **Suilend initialization is lazy** — `SuilendClient.initialize()` runs on first use (Pyth oracle setup). Cache the adapter singleton like NAVI to avoid repeated init.
5. **Enoki allowlist** — Suilend's Move call targets must be added to Enoki's `allowedMoveCallTargets` for sponsored transactions to work.
6. **`supportsSameAssetBorrow: false`** — Suilend does not support same-asset borrow. The web-app's borrow flow should respect this when the user has Suilend as their primary protocol.
7. **Multi-protocol claim rewards** — `claim-rewards` transaction must iterate all registered `LendingAdapter`s and compose their claim instructions into a single PTB. Currently NAVI-only. Suilend's `addClaimRewardsToTx` works (calls `sdk.claimRewardsAndSendToUser`), but `getPendingRewards` returns `amount: 0` because it doesn't read the `obligation.original.userRewardManagers[].rewards[].earnedRewards` field that the Suilend SDK already fetches. Fix: apply the same formula as Suilend's `getObligationClaim()` — divide `earnedRewards.value` by WAD (10^18) and mint decimals.
8. **Agent + contextual chips protocol awareness** — The agent's system prompt must instruct it to name specific protocols when discussing rates/savings. Contextual chips ("Better rate", 🏆 rewards) already reference `bestAlternativeRate.protocol` and `pendingRewards` — they auto-surface multi-protocol data once the data sources (Tasks 13.2/13.3) are updated. The morning briefing should mention per-protocol positions and rates.
...
| `t2000_swap` | [Swap] → asset grid → amount → quote → confirm | Buy, sell, swap any supported pair |
| `t2000_invest_rebalance` | **Optimize smart card** or LLM | Move investments to better protocols |
| `t2000_strategy` | [Swap] → [Strategies] | List, buy, sell strategies |
| `t2000_auto_invest` | [Swap] → [Auto] | DCA setup/status/stop |
...
| `quick-swap` | [Swap] chip | Asset grid → amount → quote |
| `investment-strategy` | [Swap] → [Strategies] | Portfolio strategies |
| `dca-advisor` | [Swap] → [Auto] | DCA setup recommendations |
