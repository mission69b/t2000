...
>
> **Status:** ✅ Shipped — Build Log & Reference (mpp.t2000.ai live)
...
2. **Order custom merch** — agent buying something for the user
   ```
   $ t2000 pay https://mpp.t2000.ai/printful/v1/order \
       --data '{"items": [{"variant_id": 4011, "quantity": 1}], "recipient": {...}}'

   ✓ Paid 18.50 USDC  ·  Tx: FjhtzF...R5AC (0.8s)
   { "id": "ord_xxx", "status": "pending", "items": 1 }
   ```
...
## Phase 3: Spec + Docs — MOVED TO suimpp.dev ✅

> **Decision:** Spec and docs pages were initially built on `mpp.t2000.ai` but have been migrated to `suimpp.dev` as the canonical protocol reference. The gateway now links externally.

**What changed:**
- Deleted `/docs` and `/spec` pages from `apps/gateway`
- Gateway header nav links to `suimpp.dev/docs` and `suimpp.dev/spec` externally
- Gateway is now 3 pages: `/` (landing), `/services` (catalog), `/explorer` (payments)
- suimpp.dev has the full spec, docs, and explorer (see `SUI_PAYMENTS_HUB.md`)

### Gateway navigation (current)

```
mpp.t2000.ai                     Services · Explorer · Docs (→ suimpp.dev) · Spec (→ suimpp.dev)
```

### Original checklist (completed on suimpp.dev, not gateway)

- [x] Spec page — render MPP charge method spec → lives at `suimpp.dev/spec`
- [x] Docs page — two-track developer guide → lives at `suimpp.dev/docs`
- [x] Code examples with copy buttons
- [x] Syntax highlighting (sugar-high)
- [x] Removed native /docs and /spec from gateway
...
### ~~Reloadly storytelling~~ — REMOVED (Phase 24a)

> Gift card integration (Reloadly) was removed in Phase 24a due to financial risk, UX friction, and codebase complexity. See `spec/COMMERCE_V2.md` for full rationale and the Pepesto integration that replaces it.

#### Marketing campaign — "Agents Can Buy Things Now"

**The angle:** Frame the outcome, not the mechanism. Nobody cares about APIs — they care that an AI did real things. The USDC/Sui/MPP pipeline is invisible. The story: **machines doing things for humans.**

**Updated campaign (post-gift-card removal):**

Focus on services that demonstrate real-world impact:
- Physical mail (postcards, letters) — tangible output
- Custom merch (Printful) — AI-designed, shipped to your door
- Image generation — creative output from a sentence
- Translation, web search, flight search — instant utility

#### Future viral content (when deferred services ship)

| Service | The tweet |
|---------|-----------|
| Suno | "My agent wrote me a song. Listen:" |
| Heygen | "My agent made a video of me presenting. I never recorded anything:" |
| Runway | "My agent turned my screenshot into a video:" |

Currently deferred (Vercel timeout, need job queue). When they ship, the content writes itself.
...
| ~~Different design system~~ | ✅ Reskinned with Agentic DS dark theme (Geist fonts, N900 bg, #00D68F accent) |
...
| `SUI_PAYMENTS_HUB.md` | Archived — features absorbed into gateway (see `spec/archive/`) |
