> **ARCHIVED — Fully implemented.** Phases 1.4 and 3.3 shipped. Advice recording, outcome checker, anomaly detection, follow-ups, and fatigue cap are all live in `apps/server/src/cron/jobs/` and Audric internal routes. This spec is preserved for design context only — do not treat file paths or architecture diagrams as current. See `audric-build-tracker.md` for ground truth.
