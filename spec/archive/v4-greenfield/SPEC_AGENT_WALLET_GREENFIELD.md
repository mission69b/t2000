...
> **Status:** ✅ **SHIPPED** 2026-05-26 — `@t2000/{sdk,engine,cli,mcp}@4.0.0` live on npm (publish workflow `26436221884` completed 2026-05-26T06:32:55Z; GitHub release `v4.0.0`; Discord #releases notified). The pivot is closed. 7-phase journey across a single session: Phase A DONE (S.328-S.335; 8/8 mainnet smokes); Phase B DONE (S.336 + S.339; Phase B audit S.340 clean); Simplification cut DONE (S.337); `--import` restored (S.338); Phase C DONE (S.340 — `t2` bin alongside `t2000`); Phase D DONE (S.341 — 3 package READMEs rewritten v3→v4 + `apps/web` Vercel redeploy smoke-passed); Phase E DONE (S.342 — 4 SPECs archived to `spec/archive/v4-greenfield/`, 4 actionable stale refs cleaned, 3 package.json descriptions+keywords refreshed v3→v4, T2000_RPC_URL + T2000_GRPC_URL env wiring verified); Phase F DONE (S.343 — `apps/docs/` Mintlify project scaffolded for `developers.t2000.ai`, flat 5-page nav per locked decision 9, all 4 SPEC verification gates pass at ≤1 click); Phase G DONE (S.344 — `gh workflow run release.yml --field bump=major` triggered the canonical bump-tag-publish chain: release.yml bumped all 4 packages 3.3.0 → 4.0.0 in 34s, publish.yml ran `pnpm publish` for `@t2000/{sdk,engine,mcp,cli}` in 1m8s + created GitHub Release v4.0.0 in 7s + Discord notification in 10s, all 4 packages confirmed at 4.0.0 via `npm view`; pre-publish verify gate green: SDK 612 + Engine 1225 + CLI 213 + MCP 64 = 2114 tests, 4-package typecheck + build clean). Founder-side follow-ups left: (a) audric `apps/web-v2` bump `@t2000/{sdk,engine}` from 3.3.0 → 4.0.0 (Vercel auto-deploys on push); (b) Mintlify dashboard connect for `developers.t2000.ai` + CNAME.
...
| **G** | Verify + release `@t2000/*@4.0.0` | ~0.5d | ✅ **DONE 2026-05-26** (S.344 — pre-flight gate green: SDK 612 + Engine 1225 + CLI 213 + MCP 64 = 2114 tests pass, 4-package typecheck clean, 4-package build clean. `gh workflow run release.yml --field bump=major` (run `26436201851`) bumped all 4 packages 3.3.0 → 4.0.0 in 34s using the canonical sdk-source-of-truth pattern + committed `📦 build: v4.0.0` (commit `3fcc7a9f`) + tagged + pushed to origin. Tag push triggered `publish.yml` (run `26436221884`) which ran CI typecheck/test/build + `pnpm publish` for all 4 packages (1m8s) + GitHub Release `v4.0.0` (7s) + Discord #releases notification (10s). All 4 packages confirmed live on npm at 4.0.0 (`npm view @t2000/{sdk,engine,cli,mcp} version`). SPEC promoted from `spec/active/shipping/` → `spec/archive/v4-greenfield/` per the closeout requirement.) |
